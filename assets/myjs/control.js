...
function selectCustomer(cid, cname,cadd2, ph, email, discount = 0,credit_period=0,credit_limit=0,avalable_credit_limit=0) {
    // function selectCustomer(cid, cname, cadd1, cadd2, ph, email, discount = 0) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.status === 'Success') {
                    $('#customer_address1').html('<strong>' + response.data + '</strong>');
                } else {
        ...
        // erp2024 remove existing customer details 29-03-2025
        // $(".existingcustomer_details").empty(); 
        $("#available_credit").remove();
        $("#avalable_credit_limit").remove();
        $(".display-section").remove();
        $('#customer_id').val(cid);
        $('#custom_discount').val(discount);
        $('#customer_name').html('<strong>' + cname + '</strong>');
        $('#customer_name').val(cname);
        $('#customer_address1').html('<strong>' + cadd2 + '</strong>');
        //erp2024 for removing credit section of an existing customer
        $('.creditsection').remove();
        $('#customer_phone').html('Phone : <strong>' + ph + '</strong><br>Email : <strong>' + email + '</strong><br>Company Credit Limit : <strong>' + credit_limit + '</strong><br>Credit Period : <strong>' + credit_period + '(Days)</strong><br><br><strong><span class="avail_creditlimit" id="avalable_credit_limit">Available Credit Limit : ' + avalable_credit_limit + '</span></strong><input type="hidden" id="available_credit" value="'+avalable_credit_limit+'">');
        $("#customer-box").val();
        $("#customer-box-result").hide();
        $(".sbox-result").hide();
        $("#customer").show();
        if($("#invoiceyoghtml").val())
        {
            ...
            if(($("#available_credit").val()) && ((pageName=="SalesOrders/draft_or_edit") || (pageName=="SalesOrders/create") || (pageName=="DeliveryNotes/create")  || (pageName=="invoices/create") || (pageName=="SalesOrders/salesorder_new") || (pageName=="deliverynotes/create") )){  
                credit_limit_with_grand_total();
            }
        }
...
function selectCustomerwithcredit(cid, cname,cadd2, ph, email, credit_limit,credit_period,avalable_credit_limit) {
    // function selectCustomer(cid, cname, cadd1, cadd2, ph, email, discount = 0) {
        $.ajax({
            type: 'POST',
            url: baseurl + 'customers/customer_details_byid',
        ...
        $('#customer_name').val(cname);
        $('#customer_address1').html('<strong>' + cadd2 + '</strong>');
        $('#customer_phone').html('Phone: <strong>' + ph + '</strong><br>Email: <strong>' + email + '<strong><br>Company Credit Limit: ' + credit_limit + '</strong><br>Credit Period: <strong>' + credit_period + '(Days)<br>Available Credit Limit: <strong>' + avalable_credit_limit + '</strong>');
        $("#customer-box").val();
        $("#customer-box-result").hide();
        $(".sbox-result").hide();
        $("#customer").show();
    ...
    function selectSupplier(cid, cname, city, ph, email) {
        $.ajax({
            type: 'POST',
            url: baseurl + 'supplier/supplier_details_byid',
            data: {'cid': cid},
            dataType: 'json',
            success: function(response) {
                if (response.status === 'Success') {
                    $('#customer_address1').html('<strong>' + response.data + '</strong><br>City: <strong>' + city + '</strong>');
                } else {
                    console.error('Failed to get customer details');
                }
            },
        ...
        $('#customer_name').html('<strong>' + cname + '</strong>');
        $('#customer_name').val(cname);
        // $('#customer_address1').html('<strong>' + city + '</strong>');
    ...
    // function isPositiveNumber(evt, input) {
    ...
    //     // Ensure only positive integers are kept (clean input on paste)
    //     setTimeout(() => {
    ...
    $("#submit-data").on("click", function (e) {
        ...
        o_data += "&completed_status=1";
        var action_url = $('#action-url').val();
        addObject(o_data, action_url);
        setTimeout(function(){  $("#submit-data").show(); }, 1000);
        setTimeout(function(){  $(".sub-btn").show(); }, 1000);
    });
    ...
    //copy the company address to billing
    $("#copy_address").change(function () {
        if ($(this).prop("checked") == true) {
            $('#billing_name').val($('#companyname').val());
            $('#billing_contact_person').val($('#mcustomer_name').val());
            $('#billing_phone').val($('#mcustomer_phone').val());
            $('#billing_email').val($('#mcustomer_email').val());
            $('#billing_address_1').val($('#mcustomer_address1').val());
            $('#billing_city').val($('#mcustomer_city').val());
            $('#billing_region').val($('#region').val());
            $('#billing_country').val($('#mcustomer_country').val()).trigger('change');
            $('#billing_postal_code').val($('#postbox').val());
        } else {
            $('#billing_name').val('');
    ...
    $("#copy_billing_address").change(function () {
        if ($(this).prop("checked") == true) {
            ...
            $('#shipping_contact_person').val($('#billing_contact_person').val());
            ...
            $('#shipping_postbox').val($('#billing_postal_code').val());
        } else {
            $('#shipping_name').val('');
            ...
            $('#shipping_postbox').val('');
        }
    });

    $("#copy_supplier_address").change(function () {
        if ($(this).prop("checked") == true) {
            // alert("Checkbox is checked." );
            $('#msupplier_name_s').val($('#msupplier_name').val());
            $('#msupplier_phone_s').val($('#msupplier_phone').val());
            $('#msupplier_email_s').val($('#msupplier_email').val());
            $('#msupplier_address1_s').val($('#msupplier_address1').val());
            $('#msupplier_city_s').val($('#msupplier_city').val());
            $('#msupplier_region_s').val($('#msupplier_region').val());
            // $('#msupplier_country_s').val($('#msupplier_country').val());
            $('#msupplier_country_s').val($('#msupplier_country').val()).trigger('change');
            $('#msupplier_postbox_s').val($('#msupplier_postbox').val());
        } else {
            $('#msupplier_name_s').val('');
            $('#msupplier_phone_s').val('');
            $('#msupplier_email_s').val('');
            $('#msupplier_address1_s').val('');
            $('#msupplier_city_s').val('');
            $('#msupplier_region_s').val('');
            $('#msupplier_country_s').val('');
            $('#shipping_postbox').val('');
        }
    });


    $("#billing_shipping_address").change(function () {
        if ($(this).prop("checked") == true) {
            // alert("Checkbox is checked." );
            ...
            $('#mcustomer_country_s').val($('.mcustomer_country').val()).trigger('change');
            $('#shipping_postbox').val($('.postbox').val());
        } else {
