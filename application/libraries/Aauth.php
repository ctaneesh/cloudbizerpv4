...
class Aauth
{
    ...
    public function login($identifier, $pass, $remember = false, $captcha = false, $totp_code = NULL)
    {
        ...
        // if ($this->config_vars['ddos_protection'] && $captcha && $this->get_login_attempts() > $this->config_vars['recaptcha_login_attempts']) {
            ...
            //captcha checking section
            if (!$resp->success) {
                $this->error($this->CI->lang->line('aauth_error_recaptcha_not_correct'));
                return FALSE;
            }
            
            // $recaptcha = $this->CI->input->post("g-recaptcha-response");
            // $secret_key = $result->recaptcha_s;
    ...
    public function apilogin($identifier, $pass, $remember = false, $captcha = false, $totp_code = NULL)
    {
    ...
    public function forgotpassword($recaptcha)
    {
    ...
    public function get_user($user_id = FALSE)
    {
        ...
        if ($query->num_rows() <= 0) {
         ...
         $this->CI->config->set_item('currency',  currency($query->row()->loc),$user_id);
        return $query->row();
    }
    ...
    public function premission($module_id)
    {
        ...
        $query = $this->aauth_db->get('cberp_premissions');
        $out=$query->row_array();
        return $out[$role];
    }
    ...
    {
        $this->aauth_db->select('cberp_pms.*,cberp_pms.id AS pid,cberp_employees.*');
        $this->aauth_db->from('cberp_pms');
        if (is_numeric($receiver_id)) {
            $query = $this->aauth_db->where('cberp_pms.receiver_id', $receiver_id);
            $query = $this->aauth_db->where('cberp_pms.pm_deleted_receiver', 0);
        }
        if (is_numeric($sender_id)) {
            $query = $this->aauth_db->where('cberp_pms.sender_id', $sender_id);
            $query = $this->aauth_db->where('cberp_pms.pm_deleted_sender', 0);
        }

        $this->aauth_db->order_by('cberp_pms.id', 'DESC');
        $this->aauth_db->join('cberp_employees', 'cberp_employees.id = cberp_pms.sender_id', 'left');
        $this->aauth_db->limit($limit, $offset);
    ...
    {
         $this->aauth_db->insert('cberp_log', array('note'=>$input1,'user'=>$input2,'created'=>date('Y-m-d H:i:s')));
    }
     ...
     public function clock()
     {
         ...
         $this->aauth_db->from('cberp_employees');
         $query = $this->aauth_db->get();
         $emp = $query->row_array();
         return $emp['clock'] ?? 0;
     }
