...
class Common
{
    ...
    function languages($id = 0)
    {
        if ($id) {
            ...
            $this->PI->db->from('cberp_locations');
            $this->PI->db->where('id', $id);
            $query = $this->PI->db->get();
            $out = $query->row_array();
        } else {
            $this->PI->db->select('*');
            $this->PI->db->from('cberp_system');
            $this->PI->db->where('id', 1);
            $query = $this->PI->db->get();
            $out = $query->row_array();
        }
        $this->PI->db->select('cberp_languages.*');
        $this->PI->db->from('cberp_languages');
        $this->PI->db->where('status', 1);
        $query1 = $this->PI->db->get();
        $languages = $query1->result_array();
        ...
        if(!empty($languages))
        {
            ...
            {
                    $sel = ($row['language_name'] == $out['lang']) ? "selected" : "" ;
                    $lang .= '<option value="' . $row['language_name'] . '" '.$sel.'>' . ucfirst($row['language_name']) . '</option>';     
            }
             return $lang;
        }
    }
    ...
    function current_language($lang = '')
    {
        ...
        if(!empty($languages))
        {
            foreach($languages as $row)
            {
                    $sel = ($row['language_name'] == $lang) ? "selected" : "" ;
                    $lang .= '<option value="' . $row['language_name'] . '" '.$sel.'>' . ucfirst($row['language_name']) . '</option>';     
            }             
        }
        return $lang;
    ...
    public function default_warehouse()
    {
        if ($this->PI->aauth->get_user()->loc) {
            ...
            $this->PI->db->select('cberp_locations.ware,cberp_store.title');
            $this->PI->db->from('cberp_locations');
            $this->PI->db->join('cberp_store', 'cberp_locations.ware=cberp_store.id', 'left');
            $this->PI->db->where('cberp_locations.id', $this->PI->aauth->get_user()->loc);
            $query = $this->PI->db->get();
            ...
            $wr = '<option value="0">*' . $this->PI->lang->line('All') . '</option>';
            $this->PI->db->select('univarsal_api.key1,cberp_store.title');
            $this->PI->db->from('univarsal_api');
            $this->PI->db->join('cberp_store', 'univarsal_api.key1=cberp_store.id', 'left');
