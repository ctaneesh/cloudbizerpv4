...
class User extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($this->input->get('e')) {
            $data['response'] = 'Invalid username or password or reCaptcha not valid!';
        }
    ...
    public function checklogin()
    {
        ...
        if ($this->aauth->login($user, $password, $rem, $this->captcha)) {
            ...
            configured_data();
            //erp2024 for load permissions of a user 15-01-2025
    ...
    public function send_reset()
    {
        ...
        if ($this->aauth->forgotpassword($recaptcha)) 
        {
            if ($out) {
    ...
    public function check_permission_ajax() {
        $permissions = $this->input->post('permissions');
        $msg = check_permission($permissions);
