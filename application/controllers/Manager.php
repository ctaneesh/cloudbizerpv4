...
class Manager extends CI_Controller
{
    ...
    public function todo_load_list()
    {
        ...
        foreach ($list as $task) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="#" class="btn btn-secondary btn-sm rounded set-task" data-id="' . $task->id . '" data-stat="0"> SET </a>' . $name;
            $row[] = $task->duedate;
            $row[] = $task->start;
            
            $row[] = '<span class="task_' . $task->status . '">' . $task->status . '</span>';

            $row[] = '<a href="#" data-id="' . $task->id . '" class="view_task btn btn-secondary btn-sm"> <i class="icon-eye"></i> </a>';
    ...
    public function pendingtasks()
    {
        ...
        foreach ($tasks as $row) {
            $target_url = ($row['target_url']) ? $row['target_url'] : "javascript:void(0)";
            $tlist .= '<a href="'.$target_url.'" class="breaklink open-task"  data-task-id="' . $row['id'] . '">
                      <div class="media">
                        <div class="media-left valign-middle"><i class="fa fa-bullhorn icon-bg-circle bg-cyan"></i></div>
                        <div class="media-body">
                          <h6 class="media-heading">' . $row['description'] . '</h6>
