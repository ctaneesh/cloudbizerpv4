...
class Settings extends CI_Controller
{
    ...
    public function company()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->settings->update_company(1, $name, $phone, $email, $address, $city, $region, $country, $postbox, $tax_id, $data_share, $foundation);
            ...
            $data['permissions'] = load_permissions('Settings','Business Settings','Manage');
    ...
    public function tax()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->settings->update_tax(1, $tax_id, $taxstatus, $gst_type);
        } else {
            $head['usernm'] = $this->aauth->get_user()->username;
    ...
    public function prefix()
    {
        ...
        if ($this->input->post()) {
            ...
            $dnote_prefix = $this->input->post('dnote_prefix', true);
            $dnote_return_prefix = $this->input->post('dnote_return_prefix', true);
            $invoicereturnprefix = $this->input->post('invoicereturnprefix', true);
            $leadprefix = $this->input->post('leadprefix', true);
            ...
            $data['company'] = $this->settings->company_details(1);
            $data['prefix'] = $this->settings->prefix();
            // echo "<pre>"; print_r($data['prefix']); die();
            $this->load->view('fixed/header', $head);
    ...
    public function discship()
    {
        ...
        if ($this->input->post()) {
            ...
            $this->plugins->update_api(61, $discstatus, $shiptax_rate, 0, $shiptax_type, $discstatus_name,$shipping_return);
    ...
    public function delete_custom_field()
    {
        ...
        if ($this->db->delete('cberp_custom_fields', array('id' => $id))) {
            $this->db->delete('cberp_custom_data', array('field_id' => $id));
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('DELETED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
        }
...
FROM cberp_invoices AS i LEFT JOIN cberp_customers AS c ON i.csd=c.id GROUP BY  i.csd ORDER BY  i.id  LIMIT 10");
        $error = $this->db->error();
        if (@$error['code']) {
    ...
    public function defaultvalidity()
    {
            $data['validity'] =$this->settings->get_default_validity();
            $data['paymentterms'] =$this->settings->get_payment_terms();
            $head['title'] = "Billing Settings";
            $head['usernm'] = $this->aauth->get_user()->username;
            $this->load->view('fixed/header', $head);
            $this->load->view('settings/default_validity', $data);
            $this->load->view('fixed/footer');
    }
    public function validity_modify_action()
    {

        $data = [
            'lead_validity' => $this->input->post('lead_validity'),
            'quote_validity' => $this->input->post('quote_validity'),
            'salesorder_validity' => $this->input->post('salesorder_validity'),
            'deliverynote_validity' => $this->input->post('deliverynote_validity'),
            'invoice_validity' => $this->input->post('invoice_validity'),
            'payment_terms' => $this->input->post('payment_terms'),
            'purchase_order_validity' => $this->input->post('purchase_order_validity'),
            'due_date_1' => $this->input->post('due_date_1'),
            'due_date_color_1' => $this->input->post('due_date_color_1'),
            'due_date_2' => $this->input->post('due_date_2'),
            'due_date_color_2' => $this->input->post('due_date_color_2'),
            'due_date_3' => $this->input->post('due_date_3'),
            'due_date_color_3' => $this->input->post('due_date_color_3'),
            'due_date_expired_color' => $this->input->post('due_date_expired_color'),
            'due_date_default_color' => $this->input->post('due_date_default_color'),
        ];
        $this->db->update('cberp_default_validity',$data);
