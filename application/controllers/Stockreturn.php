...
class Stockreturn extends CI_Controller
{
    ...
    public function create()
    {
        ...
        $stock_return_id = $this->input->get('pid');
        ...
        $head['title'] = "Stock return";
        $head['usernm'] = $this->aauth->get_user()->username;
        $data['warehouse'] = $this->stockreturn->warehouses();
        $data['taxdetails'] = $this->common->taxdetail();
        $this->load->view('fixed/header', $head);
       
        if(!empty($stock_return_id))
        {            
            $data['trackingdata'] = tracking_details('stock_return_id',$stock_return_id);
            $data['invoice'] = $this->stockreturn->purchase_details($stock_return_id);
            $data['products'] = $this->stockreturn->purchase_products($stock_return_id);
            $data['assignedperson'] = $this->stockreturn->assigedemployee($data['invoice']['assign_to']);
            $data['journals_records'] = $this->stockreturn->purchase_return_journal_records($stock_return_id);
            $data['payment_records'] = $this->stockreturn->purchase_return_payments_received($stock_return_id);
            ...
            $data['log'] = $this->stockreturn->gethistory($stock_return_id);
            $data['stockreturnid'] = $stock_return_id;
            //erp2024 06-01-2025 detailed history log starts
            $page = "Purchasereturn";
            $data['detailed_log']= get_detailed_logs($stock_return_id,$page);
            ...
            $data['return_exists'] = $this->stockreturn->get_purchase_order_by_field('id',$stock_return_id);
            $data['groupedDatas'] = $groupedBySequence;
            //  echo "<pre>"; print_r($data['groupedDatas']); die();         
            $data['images'] = get_uploaded_images('Purchasereturn',$stock_return_id);
            $this->load->view('stockreturn/newinvoice_approve', $data);
        }
        ...
        {      
            ...
            if($purchase_reciept_number)
            {
                ...
                $data['invoice'] = $this->stockreturn->purchase_receipt_details($purchase_reciept_number);   
               
                $data['products'] = $this->stockreturn->purchase_receipt_products($purchase_reciept_number);  
                // echo "<pre>"; print_r($data['invoice']); die();            
                // $data['return_exists'] = $this->stockreturn->get_purchase_order_by_field('purchase_reciept_id',$purchase_reciept_number);
            }
       ...
       //action
       public function purchase_return_action()
       {
           ...
           //erp2024 25-02-2025 from delivery returns
           ...
           if($iid>0){
               $this->db->update('cberp_stock_returnseturns', $data,['id'=>$iid]);
               $invocieno = $iid;            
               $this->db->delete('cberp_stock_returns_items', array('tid' => $invocieno));
               detailed_log_history('Purchasereturn',$invocieno,'Updated', $changedFields);
           }
           else{
               $this->db->insert('cberp_stock_returnseturns', $data);
               $invocieno = $this->db->insert_id();
               detailed_log_history('Purchasereceipt',$purchase_reciept_id,'Purchase Receipt converted to Purchase Return', $changedFields);
               detailed_log_history('Purchasereturn',$invocieno,'Created', $changedFields);
               if($invoice_id)
               {
           ...
           history_table_log('cberp_purchase_return_logs','purchase_return_id',$invocieno,'Create');
           //erp2024 06-01-2025 detailed history log starts
           ...
           if(!empty($purchase_reciept_id))
           {
               ...
               insertion_to_tracking_table('stock_return_id', $invocieno, 'stock_return_number', $this->input->post('stock_return_number', true),'purchase_reciept_id',$purchase_reciept_id);
           }
           if ($invocieno) {
               ...
               if ($prodindex > 0) {
                   ...
                   $this->db->update('cberp_stock_returnseturns');
                   $log = [
                       'stock_return_id' => $invocieno,
               ...
               if($invoice_id)
               {
                   ...
                   $this->db->update('cberp_stock_returnseturns',$stock_return_data,['id'=> $invocieno]);
                     $receivable_data = [
                         'acid' => $invoice_receivable_account_details,
                         'type' => 'Asset',
                         'cat' => 'Invoice Return',
                         'credit' => numberClean($grandprice),
    ...
    public function index()
    {
        ...
        // $condition = " WHERE i_class = 0 OR  i_class = 1";
        // $data['counts'] = $this->invoices_model->get_dynamic_count('cberp_stock_returns','invoicedate','total',$condition);
        $data['counts'] = $this->stockreturn->get_filter_count();
        $head['usernm'] = $this->aauth->get_user()->username;
        $this->load->view('fixed/header', $head);
        $this->load->view('stockreturn/invoices', $data);
        $this->load->view('fixed/footer');
    }
    ...
    public function action()
    {
        ...
        $grandsubtotal = 0;
        if (!$currency) $currency = 0; 
        $invoice_retutn_number = $this->stockreturn->lastinvoicereturn();
        $data = [
            'invoice_retutn_number'         => $invoice_retutn_number,
            'invoice_number'                => $invoice_id,
            'transaction_number'            => $transaction_number,
            'delivery_return_number'        => $delivery_return_number,
        ...
        // else{
            ...
            $invocieno = $invoice_id;
            $module_number =  module_number_name('Delivery Return');
            detailed_log_history($this->module_number,$delivery_return_number,'Converted to Credit Note', $changedFields);
            detailed_log_history('Invoicereturn',$invoice_retutn_number,'Created', $changedFields);
            if($invoice_id)
            {
                $this->db->update('cberp_invoices', ['creditnote_status'=>'Approved','invoice_retutn_number'=>$invoice_retutn_number],['invoice_number'=>$invoice_id]);
                 insertion_to_tracking_table('invoice_retutn_number',$invoice_retutn_number,'invoice_number',$invoice_id);
            }
        ...
        if ($invocieno) {
            ...
            if ($prodindex > 0) {
                ...
                $this->db->where('invoice_retutn_number', $invoice_retutn_number);
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            switch (true) {
                // stockreturn/create?pid=1&token=0ebdf108dc24b5d08ca2329c0a7048458e4b85c7
                case ($invoices->return_status == "Pending" && $invoices->approvalflg == "0"):
                    $actionbtn = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Approve Now" class="btn btn-sm btn-secondary">Approve Now</a>';
                    $approvstatus = '';
                    break;

                case ($invoices->approvalflg == "1" && $invoices->return_status == "Assigned" && $invoices->assign_to == $this->session->userdata('id')):
                    $approvstatus = '<span class="st-received">' . $this->lang->line('Approved') . '</span>';
                    $actionbtn = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Accept & Send" class="btn btn-sm btn-secondary">Accept Send</a>';
                    break;
               

                case ($invoices->approvalflg == "1" && $invoices->return_status != "Reverted" && $invoices->return_status != "Received"):
                    $approvstatus = '<span class="st-received">' . $this->lang->line('Approved') . '</span>';
                    $actionbtn = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Approve Now" class="btn btn-sm btn-secondary"><i class="fa fa-edit"></i></a>';
                    break;
                case ($invoices->return_status == "Received"):
                    $approvstatus = '<span class="st-received">' . $this->lang->line('Approved') . '</span>';
                    $actionbtn = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Approve Now" class="btn btn-sm btn-secondary"><i class="fa fa-edit"></i></a>&nbsp;<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Stock Return" class="btn btn-sm btn-secondary">Stock Return</a>';
                    break;

                case ($invoices->return_status == "Reverted"):
                    $approvstatus = '<span class="st-received">' . $this->lang->line('Approved') . '</span>';
                    $actionbtn = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '" title="Approve Now" class="btn btn-sm btn-secondary"><i class="fa fa-edit"></i></a>&nbsp;<a href="' . base_url("Invoices/costing?pid=$invoices->id") . '" title="Approve Now" class="btn btn-sm btn-secondary">Ready To Send</a>';
                    break;
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="' . base_url("stockreturn/create?pid=$invoices->id&token=$validtoken") . '">&nbsp; ' . $prefix.$invoices->tid . '</a>';
    ...
    public function editaction()
    {
        ...
        $this->db->where('id', $invocieno);
        if ($flag) {
            if ($this->db->update('cberp_stock_returns', $data)) {
        ...
        $this->db->insert('supplier_stock_return_log', $log);
        //erp2024 06-01-2025 detailed history log starts
        detailed_log_history('Purchasereturn',$invocieno,'Assign and Updated', $_POST['changedFields']);
        //erp2024 06-01-2025 detailed history log ends 
        // file upload section starts 22-01-2025
            if($_FILES['upfile'])
            {
    ...
    public function stock_return_send_by_admin_action()
    {
        ...
        if(empty($invocieno))
        {
            $data1 = array('tid' => $this->input->post('invocieno'), 'prepared_by' => $this->session->userdata('id'),'prepared_dt'=>date('Y-m-d H:i:s'),'prepared_flg'=>'1','eid'=>$this->session->userdata('id'),'purchase_reciept_id'=>$this->input->post('purchase_reciept_id'),'purchase_reciept_number'=>$purchase_reciept_number);

            $data1['created_by'] = $this->session->userdata('id');
            $data1['created_dt'] = date('Y-m-d H:i:s');

            $this->db->insert('cberp_stock_returns', $data1);
            $invocieno = $this->db->insert_id();
            insertion_to_tracking_table('stock_return_id', $invocieno, 'stock_return_number', $this->input->post('stock_return_number', true),'purchase_reciept_id',$this->input->post('purchase_reciept_id'));
        }
        ...
        foreach ($pid as $key => $value) {
            if(intval($product_qty[$key]) > 0 && !empty($product_name1[$key]))
            {
                ...
                $product_cost=0;
                // insert_data_to_average_cost_table($product_id[$key], $product_cost,numberClean($product_qty[$key]), get_costing_transation_type("Purchase Return"));
            }
        ...
        $this->db->insert('supplier_stock_return_log', $log);
        //erp2024 06-01-2025 detailed history log starts
        detailed_log_history('Purchasereturn',$invocieno,'Stock Receipt Directly sent by authorized person', $changedFields);
        //erp2024 06-01-2025 detailed history log ends 
        // file upload section starts 22-01-2025
            if($_FILES['upfile'])
            {
    ...
    public function revert_return_by_admin_action()
    {
        ...
        $purchase_id = $this->input->post('purchase_id');
        $this->db->update('cberp_stock_returns', ['assign_to' => NULL,'approved_dt' => (NULL),'approvalflg'=>'1','return_status'=>'Reverted'], ['id'=> $receipt_id]);
        $log = [
        ...
        //erp2024 06-01-2025 detailed history log starts
        detailed_log_history('Purchasereturn',$receipt_id,'Reverted By Admin', $changedFields);
        //erp2024 06-01-2025 detailed history log ends 
        echo json_encode(array('status' => 'success'));
    }
    ...
    public function revert_stock_return_by_employee_action()
    {
        $receipt_id = $this->input->post('receipt_id');
        $purchase_id = $this->input->post('purchase_id');
        $this->db->update('cberp_stock_returns', ['assign_to' => NULL,'approvalflg'=>'0','return_status'=>'Pending'], ['id'=> $receipt_id]);
        $log = [
            'stock_return_id' => $receipt_id,
            'purchase_id' => $purchase_id,
            'performed_by' => $this->session->userdata('id'),
    ...
    public function stock_return_accept_by_employee()
    {
        ...
        if ($flag) {
            if ($this->db->update('cberp_stock_returns', $data)) {
                //erp2024 06-01-2025 detailed history log starts
                detailed_log_history('Purchasereturn',$invocieno,'Accepeted by Employee', $_POST['changedFields']);
                //erp2024 06-01-2025 detailed history log ends 
    ...
    public function draftaction()
    {
        ...
        if ($this->db->insert('cberp_stock_returns', $data)) {
            $invocieno = $this->db->insert_id();
            insertion_to_tracking_table('stock_return_id', $invocieno, 'stock_return_number', $this->input->post('stock_return_number', true),'purchase_reciept_id',$purchase_reciept_id);
            $pid = $this->input->post('pid');
            $productlist = array();
            ...
            if ($prodindex > 0) {
                ...
                $this->db->insert('supplier_stock_return_log', $log);
                //erp2024 06-01-2025 detailed history log starts
                detailed_log_history('Purchasereturn',$invocieno,'Data Saved as Draft', $changedFields);
                //erp2024 06-01-2025 detailed history log ends 
                // file upload section starts 22-01-2025
                  if($_FILES['upfile'])
                  {
    ...
    public function draftaction_sub()
    {
        ...
        $grandsubtotal = 0;
        if (!$currency) $currency = 0;
        $data = array('invoicedate' => $bill_date, 'invoiceduedate' => $bill_due_date, 'subtotal' => $subtotal, 'shipping' => $shipping, 'ship_tax' => $shipping_tax, 'ship_tax_type' => $ship_taxtype, 'total' => numberClean($total), 'notes' => $notes, 'csd' => $customer_id, 'eid' => $this->aauth->get_user()->id, 'taxstatus' => $tax, 'discstatus' => $discstatus, 'format_discount' => $discountFormat, 'refer' => $refer, 'term' => $pterms, 'loc' => $this->aauth->get_user()->loc, 'i_class' => $person_type, 'multi' => $currency, 'purchase_reciept_id' => $purchase_reciept_id, 'purchase_id' => $purchase_id,'return_status'=>'Draft','purchase_order' => $purchase_order);
        
        if ($this->db->update('cberp_stock_returns', $data,['id'=>$invocieno])) {
            $this->db->delete('cberp_stock_returns_items', array('tid' => $invocieno));
            $pid = $this->input->post('pid');
            $productlist = array();
            $prodindex = 0;
            ...
            if ($prodindex > 0) {
                ...
                $this->db->insert('supplier_stock_return_log', $log);
                //erp2024 06-01-2025 detailed history log starts
                detailed_log_history('Purchasereturn',$invocieno,'Data Saved as Draft', $_POST['changedFields']);
                //erp2024 06-01-2025 detailed history log ends 
                // file upload section starts 22-01-2025
                    if($_FILES['upfile'])
                    {
    ...
    public function purchase_return_payment()
    {
        ...
        $data['invoice'] = $this->stockreturn->purchase_return_data($tid);
        // echo "<pre>"; print_r($data['invoice']); die();
