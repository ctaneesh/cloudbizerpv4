...
class Invoices extends CI_Controller
{
    ...
    private $prifix72;
    private $prifix51;
    private $acccount_module_group_number;
    ...
    {       
        ...
        $this->acccount_module_group_number =  get_module_details_by_name('Accounts');
    ...
    public function create()
    {
        ...
        $delivery_note_number = $this->input->get('dnid');
        $delivery_note_numbers = $this->input->get('dnids');
        ...
        if($invoice_number)
        {
            ...
            if($this->module_number)
            {
                ...
                $data['approval_level_users'] =  linked_user_module_approvals_by_module_number($this->acccount_module_group_number);   
                $data['my_approval_permissions'] =  linked_user_module_approvals_by_module_number($this->acccount_module_group_number,$this->session->userdata('id'));
                $data['module_number'] = $this->module_number;
            }
        }    
        else if($delivery_note_number)
        {
        ...
        if(!empty($invoice_number)){ 
            $data['invoiced_id'] = $invoice_number;
            $data['master'] = $this->invocies->invoice_details($invoice_number);
            ...
            $data['assigned_customer']  = get_customer_details_by_id($data['master']['customer_id']); 
            $data['customer'] = $this->invocies->customerByInvoiceid($invoice_number); 
            $data['images'] = get_uploaded_images('Invoice',$invoice_number);
            $data['returned_status'] = $this->invocies->check_delivered_and_return_qty_equal($invoice_number); 
            $data['payment_records'] = $this->invocies->invoice_payments_received($invoice_number);     
            $data['journals_records'] = ($data['invoice']['invoice_type']=='Deliverynote') ? $this->invocies->get_deliverynote_invoice_transaction_details($data['invoice']['invoice_number'])
            :$this->invocies->get_invoice_transaction_details($invoice_number);
            ...
            //  echo "<pre>"; print_r($data['merged_deliverynote']); die();
              //erp2024 06-01-2025 detailed history log starts
            $page = "invoice";
            $data['detailed_log']= get_detailed_logs($invoice_number,$page);
            $products = $data['detailed_log'];
            $groupedBySequence = []; 
            foreach ($products as $product) {
                $sequence = $product['seqence_number'];
        ...
        if($delivery_note_number)
        {
            ...
            $data['customer'] = $this->invocies->customerByDeliverynoteid($delivery_note_number);
    ...
    public function ajax_list()
    {
        ...
        foreach ($list as $invoices) {
            ...
            // $targeturl = ($invoices->status!='Draft') ? '<a href="' . base_url("invoices/view?id=$invoices->invoice_number") . '">' . $invoicetid . '</a>' : '<a href="' . base_url("invoices/create?id=$invoices->invoice_number") . '">' . $invoicetid . '</a>' ;
            ...
            $creditnoteBtn = '<a href="' . base_url("invoicecreditnotes/create?id=$invoices->invoice_number") . '" class="btn btn-sm btn-secondary ' . $disablecls . '"><i class="fa fa-undo"></i> ' . $this->lang->line('Return Items') . '</a>';
            ...
            // $actionbtn = ($invoices->status=='Draft') ? "" : '<a href="' . base_url("invoices/printinvoice?id=$invoices->invoice_number") . '&d=1" class="btn btn-secondary btn-sm"  title="Download"><span class="fa fa-download"></span></a> <a href="#" data-object-id="' . $invoices->invoice_number . '" class="btn btn-secondary btn-sm delete-object"><span class="fa fa-trash"></span></a>&nbsp;'.$makepaymentbtn." ".$creditnoteBtn; 
            $row[] = $actionbtn;
    ...
    public function action()
    {
        ...
        $invoice_action_type = $this->input->post('invoice_action_type');
        $invoice_number = ($invoice_action_type) ? $this->input->post('invoice_number') : $this->invocies->lastinvoice();
        ...
        if ($invoice_number) {
            ...
            // print_r($product_cost); die(); $this->db->set('lastbal', 'lastbal - ' . $actulprice, FALSE);
            ...
            if($deleted_items_array)
            {
                ...
                $this->db->delete('cberp_invoice_items'); 
            }
            ...
            foreach ($product_hsn as $key => $value) {
                ...
                if ($product_hsn[$key]) {
                    ...
                    $producthsns[$prodindex] =  $product_hsn[$key];
                    ...
                    if(empty($delevery_note_id) && $invoice_type !="Sales Order")
                    {
                        ...
                        insertion_to_tracking_table('invoice_number', $invoice_number,'deliverynote_number',$delevery_note_id[0]);
            ...
            if ($prodindex > 0) {
               ...
               // history_table_log('cberp_invoice_log','invoice_id',$invocietid,'Update');
                ...
                else
                {
                    ...
                    $this->db->set('status', 'Invoiced');
                    $this->db->where_in('product_code', $producthsns);
                    $this->db->where_in('delivery_note_number', $delevery_note_id);
                    $this->db->update('cberp_delivery_note_items');
        ...
        if ($transok) {
            ...
            // }
            // $data = array('type' => 9, 'rid' => $invoice_number, 'col1' => $t_profit, 'd_date' => $bill_date);
    ...
    public function convert_salesorder_to_invoice_action()
    {
        ...
        $invocietid = $this->invocies->lastinvoice();
        $invoicedate = $this->input->post('invoicedate');
        $invocieduedate = $this->input->post('invocieduedate');
        $notes = $this->input->post('notes', true);
        $tax = $this->input->post('tax_handle');
        ...
        $data['invoice_date'] = date('Y-m-d H:i:s');
        $this->db->insert('cberp_invoices',$data);
        detailed_log_history('invoice',$invoice_number,'Data Saved As Draft', "");
        insertion_to_tracking_table('invoice_number', $invoice_number,'salesorder_number',$salesorder_number);
        // file upload section starts 22-01-2025
        if($_FILES['upfile'])
        {
            upload_files($_FILES['upfile'], 'Invoice',$invoice_number);
        ...
        if ($invoice_number) {
            ...
            if ($prodindex > 0) {
                $this->db->insert_batch('cberp_invoice_items', $productlist);    
                $order_discount_percentage = order_discount_percentage($order_discount,$grandprice);
                $this->db->set(array('total_discount' => $total_discount, 'tax' => rev_amountExchange_s(amountFormat_general($total_tax), $currency, $this->aauth->get_user()->loc), 'order_discount_percentage'=>$order_discount_percentage));
                $this->db->where('invoice_number', $invoice_number);
                $this->db->update('cberp_invoices'); 
                if($salesorder_number)
                {   
                    $this->db->update('cberp_sales_orders',['converted_status'=>5,'status'=>'Invoiced'], ['salesorder_number'=>$salesorder_number]);
                }
            ...
            if ($transok) {
                $validtoken = hash_hmac('ripemd160', $invoice_number, $this->config->item('encryption_key'));
                $link = base_url('billing/printinvoice_preview?id=' . $invoice_number . '&token=' . $validtoken);
                //need view 
                // $link = base_url('billing/view?id=' . $invocieno . '&token=' . $validtoken);
                echo json_encode(array('status' => 'Success','id'=>$invoice_number,'link'=>$link));
            }
    ...
    public function editaction()
    {
        ...
        $i = 0;
        if ($this->limited) {
            $employee = $this->invocies->invoice_details($invoice_number, $this->limited);
        ...
        $data = array('subtotal' => $subtotal, 'shipping' => $shipping, 'shipping_tax' => $shipping_tax, 'shipping_tax_type' => $ship_taxtype, 'discount_rate' => $disc_val, 'total' => $total, 'notes' => $notes, 'customer_id' => $customer_id, 'employee_id' => $this->session->userdata('id'), 'tax_status' => $tax, 'discount_status' => $discstatus, 'format_discount' => $discountFormat, 'reference' => $refer, 'payment_terms' => $pterms, 'order_discount'=>$order_discount,'store_id'=>$store_id,'transaction_number'=>$transaction_number,'status'=>$status1,'payment_type'=>$payment_type);

        $this->db->set($data);
        $this->db->where('invoice_number', $invoice_number);
        $this->db->update('cberp_invoices', $data);
        ...
        if ($this->db->update('cberp_invoices', $data)) {
            ...
            if($_FILES['upfile'])
            {
                upload_files($_FILES['upfile'], 'Invoice',$invoice_number);
                 ...
                 // file upload section ends 22-01-2025
            $sequence_number = detailed_log_history('Invoice',$invoice_number,'Updated', $_POST['changedFields']);	
            // erp2025 09-01-2025 ends
            $pid = $this->input->post('pid');
            $productlist = array();
            $wholeproducttransdata = array();
            ...
            // $this->db->delete('cberp_invoice_items', array('invoice_number' => $invoice_number));
            $product_qty = $this->input->post('product_qty');
            ...
            $total_records  = count($product_hsn);
            $product_wise_order_discount = ($order_discount>0) ?round(($order_discount/$total_records),2):0;
            foreach ($product_hsn as $key => $value) {
                if($discount_type[$key]=="Amttype"){
                    $discountamount = numberClean($product_amt[$key]);
            ...
            if ($prodindex > 0) {
                ...
                // $this->db->set(array('discount' => rev_amountExchange_s(amountFormat_general($total_discount), $currency, $this->aauth->get_user()->loc), 'tax' => rev_amountExchange_s(amountFormat_general($total_tax), $currency, $this->aauth->get_user()->loc), 'order_discount_percentage'=>$order_discount_percentage,'shipping_percentage'=>$shipping_percentage));
                // $this->db->where('id', $invocie_id);
                // $this->db->update('cberp_invoices');
                ...
                // if($transok)    
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('Invoice has  been updated') . " <a href='view?id=$iid' class='btn btn-secondary btn-sm'><span class='fa fa-eye' aria-hidden='true'></span></a> "));
            } 
            else {
                echo json_encode(array('status' => 'Error', 'message' =>
                    $this->lang->line('ERROR')));
    ...
    public function customer_payment()
    {
        ...
        if ($data['invoice']['invoice_number']) {
            $data['invoice']['invoice_number'] = $tid;            
            // $data['trackingdata'] = tracking_details('invoice_id',$tid);
    ...
    public function cancelinvoiceaction()
    {
        ...
        $invoicedetails = $this->invocies->check_invoice_ispaid($invoice_number);
        ...
        $transaction_data = $this->invocies->transaction_number_invoiceid($invoice_number); 
        ...
        // history_table_log('cberp_invoice_log','invoice_id',$invoice_number,'Cancelled');
        // erp2025 09-01-2025 starts

        detailed_log_history('Invoice',$invoice_number,'Cancelled', $_POST['changedFields']);	
        // erp2025 09-01-2025 ends
        echo json_encode(array('status' => 'Success'));
    }
    ...
    public function invoicereturn_print()
    {
        ...
        // ==================================================================
        $this->db->select('cberp_stock_returns.*, cberp_stock_returns_items.*,cberp_products.product_code,cberp_product_description.product_name,cberp_products.unit AS productunit,cberp_invoices.invoice_number');
        $this->db->from('cberp_stock_returns');
        $this->db->join('cberp_stock_returns_items', 'cberp_stock_returns.invoice_retutn_number = cberp_stock_returns_items.invoice_retutn_number');
        ...
        if ($this->input->server('REQUEST_METHOD') === 'POST') {
            ...
            if(!empty($enquiry_data)){
                ...
                {                    
                    ...
                    $enquiry_data['lead_number'] = $this->prifix72['lead_prefix'].$this->invocies->lastenquiry();
                    $lead_number = $enquiry_data['lead_number'];
               ...
               $this->db->insert('cberp_transaction_tracking',['lead_id'=>$lead_id,'lead_number'=>$lead_number,'quote_number'=>$quote_number]);
               ...
               ////////////////////////////////////////////////////////////////////////////////////////
                ...
                {
                    $uploaded_data['lead_id'] = $lead_id;
                    $uploaded_data['lead_number'] = $lead_number;
                    foreach ($files['name'] as $key => $filename) {
                        $_FILES['userfile']['name'] = $files['name'][$key];
                        $uploaded_data['actual_name'] = $files['name'][$key];
                        $_FILES['userfile']['type'] = $files['type'][$key];
    ...
    public function save_as_draft_action()
    {
        // ini_set('display_errors', 1);
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);
            ...
            foreach ($product_hsn as $key => $value) {            
                if ($product_hsn[$key]) {
                    ...
                    if($isChanged && in_array($code, $product_hsn)) {
                        $this->db->update('cberp_invoice_items', $data, ['invoice_number'=>$invoice_number, 'product_code'=>$code]);
                    }
                    elseif (!$isInWhole && in_array($code, $product_hsn)) 
                    {
                        $this->db->insert('cberp_invoice_items', $data);
                    }
                    $existornot = $this->invocies->check_product_existornot($invoice_number,$product_hsn[$key]);
                    if($existornot==1)
                    {
                        $this->db->update('cberp_invoice_items', $data, ['invoice_number'=>$invoice_number, 'product_code'=>$product_hsn[$key]]);
                    }
                    else{
                        $this->db->insert('cberp_invoice_items', $data);
                    }
