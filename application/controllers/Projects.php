...
class Projects extends CI_Controller
{
    ...
    {
        $data['permissions'] = load_permissions('Project','Project Management','Manage Projects','List');
    ...
    public function addproject()
    {
        ...
        $data['permissions'] = load_permissions('Project','Project Management','Manage Projects','','Add New');
        if ($this->input->post()) {
            ...
            if ($this->projects->addproject($name, $status, $priority, $progress, $customer, $sdate, $edate, $tag, $phase, $content, $budget, $customerview, $customercomment, $link_to_cal, $color, $ptype, $employee)) {
                echo json_encode(array('status' => 'Success', 'message' => '[Project] ' . $this->lang->line('ADDED') . ' <a href="' . base_url('projects') . '" class="btn btn-secondary btn-sm breaklink"><span class="fa fa-list-alt" aria-hidden="true"></span> </a>' . ' <a href="' . base_url('projects/addproject') . '" class="btn btn-secondary btn-sm breaklink"><span class="fa fa-plus-circle" aria-hidden="true"></span> </a>'));
            } else {
    ...
    public function edit()
    {
        ...
        $data['permissions'] = load_permissions('Project','Project Management','Manage Projects','','Edit');
        if ($this->input->post()) {
            ...
            if ($this->projects->editproject($pid, $name, $status, $priority, $progress, $customer, $sdate, $edate, $tag, $phase, $content, $budget, $customerview, $customercomment, $link_to_cal, $color, $ptype, $employee)) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => '[Project] ' . $this->lang->line('UPDATED') . ' <a href="' . base_url('projects') . '" class="btn btn-secondary btn-sm breaklink"><span class="fa fa-list-alt" aria-hidden="true"></span> </a>' . ' <a href="' . base_url('projects/addproject') . '" class="btn btn-secondary btn-sm breaklink"><span class="fa fa-plus-circle" aria-hidden="true"></span> </a>'));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
    ...
    public function addmilestone()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->projects->add_milestone($name, $stdate, $tdate, $content, $color, $prid)) {
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . '&nbsp; Return to project <a href="' . base_url("projects/explore?id=" . $prid) . '" class="btn btn-secondary btn-sm"><i class="icon-eye"></i></a>'));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
    ...
    public function edittask()
    {
        if ($this->input->post()) {
            ...
            if ($this->projects->edittask($id, $name, $status, $priority, $stdate, $tdate, $employee, $content)) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('UPDATED') . '&nbsp; Return to project <a href="' . base_url("projects/explore?id=" . $prid) . '" class="btn btn-secondary btn-sm"><i class="icon-eye"></i></a>'));
            } else {
                echo json_encode(array('status' => 'Error', 'message' => $this->lang->line('ERROR')));
            }
        } else {
    ...
    public function save_addtask()
    {
        ...
        if ($this->projects->addtask($name, $status, $priority, $stdate, $tdate, $employee, $assign, $content, $prid, $milestone)) {
            ...
            echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('New Task Added') . '&nbsp; Return to project <a href="' . base_url("projects/explore?id=" . $prid) . '" class="btn btn-secondary btn-sm"><i class="icon-eye"></i></a>'));
    ...
    public function todo_load_list()
    {
        ...
        foreach ($list as $task) {
            ...
            $row[] = '<a class="btn btn-secondary btn-sm" href="' . base_url('projects') . '/edittask?id=' . $task->id . '" data-object-id="' . $task->id . '"> <i class="icon-pencil"></i> </a>&nbsp;<a class="btn btn-secondary btn-sm delete-custom" data-did="3" href="#"  data-object-id="' . $task->id . '"> <i class="fa fa-trash"></i> </a>';
