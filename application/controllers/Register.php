...
class Register extends CI_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->input->post()) {
            ...
            if ($this->registerlog->create($this->aauth->get_user()->id, $cash, $card, $bank, $cheque)) {
                ...
                echo json_encode(array('status' => 'Success', 'message' => $this->lang->line('ADDED') . " <a href='" . base_url() . "pos_invoices/create' class='btn btn-info'><span class='icon-plus-circle' aria-hidden='true'></span> " . $this->lang->line('POS') . "  </a>"));
    ...
    public function delete_i()
    {
        ...
        if ($id) {
            ...
            $this->db->delete('cberp_register', array('id' => $id));
    ...
    public function load_list()
    {
        ...
        foreach ($list as $promo) {
            ...
            $row = array();
            $row[] = $no;
            $row[] = '<a href="#" class="set-reg" data-id="' . $promo->id . '" data-stat="0"  data-toggle="modal" data-target="#view_register" >'.$promo->username.'</a>';
            $row[] = (!empty($promo->o_date))? date('d-m-Y H:i:s', strtotime($promo->o_date)):"";
            $row[] = (!empty($promo->c_date))? date('d-m-Y H:i:s', strtotime($promo->c_date)):"";;
            $row[] = $promo_status;
            $row[] = ' <a href="#" data-object-id="' . $promo->id . '" class="btn btn-secondary btn-sm delete-object" title="Delete"><span class="fa fa-trash"></span></a>';
            $data[] = $row;
        }
