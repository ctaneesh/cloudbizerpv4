        ...
        <div class="card card-block">
            <form method="post" id="data_form" class="form-horizontal">
                <div class="form-group row">                
                    <!-- ==================== header section ends ========================= -->
                    <input type="hidden" class="form-control margin-bottom  required"  name="closing_balance" id="closing_balance" value="<?=$closing_balance?>">
                     <div class="d-none">
                    <div class="col-lg-2 col-md-4 col-sm-12 col-xs-12">
                        <input type="hidden" id="reconciliations_id" name="reconciliations_id" value="<?=$reconciliations_id?>">
                        <label class="col-form-label" for="name"><?php echo $this->lang->line('Start Date') ?><span class="compulsoryfld">*</span></label>
                        <input type="date" class="form-control margin-bottom  required" name="start_date" id="start_date" value="<?=$datefrom?>" max="<?=date('Y-m-d')?>">
                    </div>
                    <div class="col-lg-2 col-md-4 col-sm-12 col-xs-12">
                        <label class="col-form-label" for="name"><?php echo $this->lang->line('End Date') ?><span class="compulsoryfld">*</span></label>
                        <input type="date" class="form-control margin-bottom  required"  name="end_date" id="end_date" value="<?=$dateto?>"  max="<?=date('Y-m-d')?>">
                    </div>
                    <div class="col-lg-2 col-md-4 col-sm-12 col-xs-12">
                        <label class="col-form-label" for="name"><?php echo $this->lang->line('Closing Balance') ?><span class="compulsoryfld">*</span></label>
                        <input type="number" class="form-control margin-bottom  required"  name="closing_balance" id="closing_balance" value="<?=$closing_balance?>">
                    </div>
                    <div class="col-lg-2 col-md-4 col-sm-12 col-xs-12">
                        ...
                        <div class="col-lg-2 col-md-4 col-sm-12 col-xs-12 mt-32px">
                            <button id="transaction_search_btn" type="button" class="btn  btn-secondary"><?php echo $this->lang->line('Search Transactions') ?></button>
                            <input type="hidden" name="id" value="0">
                        </div>
                    
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <label class="col-form-label" for="name"><?php echo $this->lang->line('Note') ?><span class="compulsoryfld">*</span></label>
                          <textarea name="note" id="note" class="form-textarea" placeholder="<?php echo $this->lang->line('Note') ?>"><?=$note?></textarea>
                        </div>
                    </div>
                <!-- ==================== header section ends ========================= -->
                                ...
                                <tbody id="journal-rows">
                                    ...
                                    {  
                                        ...
                                        foreach($banktransactions as $row)
                                        {
                                            ...
                                            echo "<td><input type='hidden' name='trans_amount[]' id='trans_amount-$i' value='$trans_amount'><input type='checkbox'class='clear_check' id='clear_check-$i' $checked><input type='hidden' name='checkedflg[]' id='checkedflg-$i' value='$checkedflg'></td>";
                                            echo "</tr>";
                                            $i++;
                                        }
                                    }
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                            <div class="row">
                                ...
                                <div class="col-3 text-right">
                                    ...
                                    <?php 
                                        $opening_balance = ($opening_balance>0) ? $opening_balance:0.00;
                                    ?>
                                ...
                                <div class="col-3 text-right">
                                    <label  class="col-form-label" id="closing_balance_text"><?=$closing_balance?></label>                                    
                                    <input type="hidden" id="closing_balance_val" name="closing_balance_val" value="<?=$closing_balance?>">
                                </div>
                                <div class="col-9 text-right">
                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 mt-2 text-right">
                        <input type="submit" id="reconcile-btn" class="btn btn-primary btn-lg margin-bottom disable-class"
...
<script type="text/javascript">
        ...
        if ($("#data_form").valid()) {                
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: baseurl + 'reconciliations/editaction',
                        type: 'POST',
                        data: formData,
                        contentType: false, 
                        processData: false,
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location.href = baseurl + 'reconciliations/edit?id=' + $("#reconciliations_id").val()+'&datefrom=' + $("#start_date").val() + '&dateto=' + $("#end_date").val() + '&bank_account=' + $("#bank_account").val();
...
$(document).ready(function() {
        ...
        function updateClearedAmount() {
            ...
            DifferenceAmount = closing_balance - Math.abs(cleared_amount);
