      ...
      <div class="card-header border-bottom">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?php echo $this->lang->line('Purchase Return Payment'); ?></li>
                </ol>
            </nav>
         ...
         <div class="row">
               <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
                  <h4 class="card-title">
                     <?php echo $this->lang->line('Purchase Return Payment'); ?>
         ...
         <div id="invoice-template" class="card-body">
            <div class="row">
               <div class="col-12">
                  <?php
                     ...
                     $link = base_url('purchasereturns/create?pid=' . $invoice['receipt_return_number'] . '&token=' . $validtoken);
                 ?>
               </div>
            </div>
            ...
            <form class="payment" name="customerPaymentForm" id="customerPaymentForm" method="POST">
               <div id="invoice-customer-details" class="row">
                  ...
                  <div class="col-lg-4 col-md-12 col-sm-12 text-xs-center text-md-left">
                        <?php 
                        ...
                        <?php 
                           $balanceamount = $invoice['subtotal'] - $invoice['payment_recieved_amount'];
                           $billdate = (!empty($invoice['invoicedate'])) ? date('d-m-Y',strtotime($invoice['invoicedate'])):"";
                           // echo '<p class="text-muted mt-2"><u>'.$this->lang->line('Purchase Receipt').$this->lang->line('Details').'</u></p>';
                           // echo $this->lang->line('Purchase Receipt No.')." : &nbsp;<a href='".$link."'>".$invoice['srv']."</a></b>\n<br>"; 
                           ...
                           // echo $this->lang->line('Paid Amount')." &nbsp;: &nbsp;<b>" .number_format($invoice['payment_recieved_amount'],2)."</b>\n<br>";
            ...
            <!--/ Invoice Customer Details -->
                     ...
                     <div class="mb-3 mt-3 table-scroll">
                        <h4><?php echo $this->lang->line('Purchase Return').' '.$this->lang->line('Details'); ?></h4>
                        ...
                        <?php //echo $this->lang->line("Select atleast one invoice"); ?>
                           ...
                           <table class="table table-striped table-bordered zero-configuration dataTable"
                              ...
                              <thead>
                                 <tr>
                                       <th width="5%" class="text-center">
                                          <?php echo $this->lang->line("Purchase Return")."#"; ?></th>
                                       <th width="5%" class="text-center">
                                          <?php echo $this->lang->line("Receipt Date"); ?></th>
                                       <th width="5%" class="text-center">
                                          <?php echo $this->lang->line("Purchase Receipt"); ?></th>
                              ...
                              <tbody>
                                 ...
                                 if(!empty($invoice)){
                                    $balanceamount = $invoice['subtotal'] - $invoice['payment_recieved_amount'];
                                    $billdate = (!empty($invoice['return_date'])) ? dateformat($invoice['invoicedate']):"";
                                    $totalval = 0;
                                       $paymentamount = $invoice['subtotal'] - $invoice['payment_recieved_amount'];
                                    //    $invoice_number = ($transaction_data['invoice_number']) ? $transaction_data['invoice_number'] : $transaction_data['invoiceumber'];
                                       echo "<tr>"; 
                                       echo "<td class='text-center'><a href='".$link."'>".$invoice['receipt_return_number']."</a></td>";
                                       echo "<td class='text-center'>".$billdate."</td>";
                                       echo "<td class='text-center'>".$invoice['purchase_reciept_number']."</td>";
                                       echo "<td class='text-right'>".number_format($invoice['subtotal'],2)."</td>";
                                       echo "<td class='text-right'>".number_format($invoice['payment_recieved_amount'],2)."</td>";
            ...
            <div class="col-12 text-right">
               <div class="form-group">
                  <input type="hidden" class="form-control required"  name="module_number" id="module_number" value="<?php echo $module_number; ?>">
                  <input type="hidden" class="form-control required"  name="tid" id="invoiceid" value="<?php echo $invoice['iid'] ?>">
                  <input type="hidden" class="form-control required"  name="receipt_number" id="receipt_number" value="<?php echo $invoice['tid'] ?>">
                  <input type="hidden" class="form-control required" name="receipt_return_number" id="receipt_return_number" value="<?php echo $invoice['receipt_return_number'] ?>">
                  <input type="hidden" class="form-control required"  name="purchase_return_id" id="purchase_return_id" value="<?php echo $invoice['iid'] ?>">
                  <input type="hidden" class="form-control required"  name="purchase_reciept_id" id="purchase_reciept_id" value="<?php echo $invoice['purchase_reciept_id'] ?>">
                  ...
                  <button type="submit" class="btn btn-lg btn-primary btn-crud" id="payment_receives_btn"><?php echo $this->lang->line('Payment Receives'); ?></button>
                  <!-- <button type="button" class="btn btn-lg btn-primary" id="submitpayment"><?php echo $this->lang->line(' Payment'); ?></button> -->
               </div>
            </div>
         <!-- ======================================================== -->
         </form>
...
<div id="cancel_bill" class="modal fade">
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <button type="button" class="btn purchase_return_payment btn-md btn-default"
            data-dismiss="modal"><?php echo $this->lang->line('Close'); ?></button>
         <button type="button" class="btn purchase_return_payment btn-md btn-primary"
...
$(document).ready(function(){
 ...
 document.querySelectorAll('input, textarea, select').forEach((input) => {
  ...
  $("#bank_account").select2({
   ...
   $('#payment_receives_btn').on('click', function(e) {
    e.preventDefault(); // Prevent the default form submission
   //  $('#payment_receives_btn').prop('disabled',true);
    // Validate the form    
    ...
    if ($("#customerPaymentForm").valid()) {
      ...
      if (receipt_amount == 0) {
         Swal.fire({
            icon: 'warning',
            title: 'Warning',
            text: 'Selected Atleat an item from the list',
      ...
      if (parseFloat(receipt_amount) < parseFloat(recievedamt)) {
        ...
        Swal.fire({
                  title: "Are you sure?",
                  text: "Do you want to receive payment now?",
                  icon: "question",
                  ...
                  if (result.isConfirmed) {
                     $.ajax({
                           url: baseurl + "transactions/purchasereturn_receivepayment",
                           type: 'POST',
                           data: formData,
                           contentType: false,
                           processData: false,
                     ...
                     $('#payment_receives_btn').prop('disabled', false);
                  }
               });
