                ...
                <form method="post" id="data_form">
                                    ...
                                    <div class="fcol-sm-12">
                                        <h4 class="card-title">
                                            <?php echo $this->lang->line('Bill To') ?> 
                                             <input type="button" value="Add New Customer" class="btn btn-sm btn-primary add_customer_btn" autocomplete="off" title="Add New Customer" >
                    ...
                    <div id="saman-row" class="table-scroll">
                        <table class="table table-striped table-bordered zero-configuration dataTable">
                            ...
                            <tr class="sub_c" style="display: table-row;">
                                <td colspan="2" class="no-border"><?php echo $this->lang->line('Payment Terms') ?> <select name="pterms"
                                                                                                         class="selectpicker form-control"><?php foreach ($terms as $row) {
                                            echo '<option value="' . $row['id'] . '">' . $row['title'] . '</option>';
                                        } ?>

                                    </select></td>
                                <td align="right" colspan="6" class="no-border"><input type="submit" class="btn btn-lg btn-primary sub-btn"
...
<script>
    $("#data_form").validate({
        ignore: [],
        rules: {      
            applyDiscount: { required: true },
        },
        messages: {
            applyDiscount: "Select Support Tickets Module",  
        },
        errorElement: "em",
    ...
    $("#submit-btn").on("click", function (e) {
        ...
        if ($("#data_form").valid()) {
            ...
            Swal.fire({
               title: "Are you sure?",
               text: "Do you want to create a new subscription?",
               icon: "question",
               ...
               if (result.isConfirmed) {  
                    $.ajax({
                        type: 'POST',
                        url: baseurl + 'subscriptions/action',
                        data: formData,
                        contentType: false,
                        processData: false,
                        dataType: "json",
                        success: function(response) {
                            window.location.href = baseurl + 'subscriptions';
                        },
