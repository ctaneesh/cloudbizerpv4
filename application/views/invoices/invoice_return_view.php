   ...
   <div class="card">
         ...
         <?php
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page"><?php echo $prefix.$invoice['tid']; ?></li>
                </ol>
            </nav>
        
         <div class="row">
               <div class="col-xl-3 col-lg-2 col-md-2 col-sm-12 col-xs-12">
                  <h4 class="card-title"><?php echo $prefix.$invoice['tid']; ?></h4>
               </div>
      ...
      <div class="card-content mt-1">
         ...
         <div id="invoice-template" class="card-body">
            <div class="row">
               <div class="col-12">
               <input type="hidden" class="form-control" name="invoiceid" id="invoice_id" value="<?php echo $invoice['returnid'] ?>">
                  <?php
                     $validtoken = hash_hmac('ripemd160', $invoice['returnid'], $this->config->item('encryption_key'));                     
                     $link = base_url('billing/view?id=' . $invoice['returnid'] . '&token=' . $validtoken);
                      ?>
                  <div class="title-action row"> 
                    <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">                  
                        <a href="<?php echo 'edit?id=' . $invoice['returnid']; ?>" class="btn btn-sm btn-secondary mb-1"><i
                            class="fa fa-pencil"></i> <?php echo $this->lang->line('Edit') ?></a>
                        <?php
                        $disablecls ="";
                        if($invoice['payment_status']=='Paid' || $invoice['payment_status']=='post dated cheque')
                        { 
                           $disablecls = "disable-class";
                        }
                         ?>
                        <a href="<?php echo base_url('invoices/payment_return_to_customer?id=' . $invoice['returnid'] . '&csd=' . $invoice['cid']); ?>" class="btn btn-sm btn-secondary mb-1 <?=$disablecls?>" title="Make Payment" ><span class="fa fa-money"></span> <?php echo $this->lang->line('Make Payment') ?> </a>

                        <!-- <a href="<?php echo 'edit?id=' . $invoice['returnid']; ?>" class="btn btn-sm btn-secondary mb-1 <?=$disablecls?>" title="Make Payment" ><span class="fa fa-money"></span> <?php echo $this->lang->line('Make Payment') ?> </a> -->
                        
                        <!-- <a href="#part_payment" data-toggle="modal" data-remote="false" data-type="reminder"
                            class="btn btn-sm btn-secondary mb-1" title="Partial Payment"
                        ...
                        <!-- <button class="btn btn-sm btn-secondary mb-1 cancelinvoice-btn"><?php echo $this->lang->line('Cancel') ?></button> -->
                    ...
                    <div class="col-lg-4 col-md-4 col-sm-12 col-xs-12 text-lg-right text-md-right text-sm-left text-xs-left">
                        ...
                        <div class="btn-group ">
                            ...
                            <a class="dropdown-item"
                                href="<?= base_url('invoices/invoicereturn_print?delivery=' . $invoice['returnid'] . '&token=' . $validtoken); ?>" target="_blank"><?php echo $this->lang->line('Print') ?></a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item"
                                href="<?= base_url('invoices/invoicereturn_print?delivery=' . $invoice['returnid'] . '&token=' . $validtoken); ?>&d=1"  target="_blank"><?php echo $this->lang->line('PDF Download') ?></a>
            ...
            <div id="invoice-company-details" class="row">
               ...
               <div class="col-md-6 col-sm-12 text-xs-center text-md-right">
                  ...
                  <ul class="px-0 list-unstyled">
                     ...
                     <?php if($invoice['payment_status']!= 'Dew')
                     { ?>
                     <li>
                        <?php echo $this->lang->line('Paid Amount').' : ' ?>
                        <span class="lead text-bold-800"><?php //echo amountExchange($invoice['total'], 0, $this->aauth->get_user()->loc) ?><?php echo '<b>'.number_format($invoice['total'],2).'</b>'; ?></span>
                     </li>
                     <?php } ?>
                  </ul>
               </div>
                                 ...
                                 <div class="table-responsive col-sm-12">
                                    <table class="table table-striped table-bordered zero-configuration dataTable">
                                          ...
                                          <?php $c = 1;
                                             ...
                                             foreach ($products as $row) {
                                                ...
                                                echo '<tr>
                                                   ...
                                                   <td>' . number_format($row['totaldiscount'], 2) . ' (' . amountFormat_s($row['discount']) . $this->lang->line($invoice['format_discount']) . ')</td>
                                       ...
                                       <tr>
                                          ...
                                          <th class="text-center"><?php echo $this->lang->line('Return Quantity') ?></th>
                                             ...
                                             foreach ($products as $row) {
                                                ...
                                                echo '<tr><th scope="row">' . $c . '</th>                     
                                                   ...
                                                   <td class="text-center">' . intval($row['qty']) ." ". $row['unit'] . '</td>';
                                                      ...
                                                      echo '<td  class="text-right">' . number_format($row['totaldiscount'],2) .'</td>
                                    ...
                                    <div class="table-responsive">
                                       <table class="table">
                                          <tbody>
                                             ...
                                             <tr>
                                                <td class="text-bold-800"><?php echo $this->lang->line('Total') ?> (<?php echo $this->config->item('currency') ?>)</td>
                                                <td class="text-bold-800 text-right"><b><?php echo number_format($invoice['total'],2); ?></b></td>
                                             </tr>
...
<button class="history-expand-button">
   ...
   <div class="history-container">
      ...
      <table id="log" class="table table-striped table-bordered zero-configuration dataTable">
         ...
         <tbody>
         <?php $i = 1;
         foreach ($groupedInvoicereturns as $seqence_number => $Invreturns){
         $flag=0;
         ?>              
               <tr>
                  <td>        
                  <?php    foreach ($Invreturns as $Invreturn) {
                  if($flag==0)
                  {?>
                  <div class="userdata">
                  <img class="rounded-circle1" src='<?php echo base_url('userfiles/employee/thumbnail/'.$Invreturn['picture'])?>' style="width:50px; height:50px;" ?>
                  <?php  echo $Invreturn['name'];
                           $flag=1;
                  } ?>
                  </div>           
                     <ul><li>  <?php echo $Invreturn['old_value'];?> > <b><span class="newdata"><?php echo $Invreturn['new_value']?></span></b> (<?php if($Invreturn['field_label']==""){echo $Invreturn['field_name'];}else{echo $Invreturn['field_label'];}?>)
                     </li></ul>
                     <?php } ?>
                  </td>               
                  <td><?php echo date('d-m-Y H:i:s', strtotime($Invreturn['changed_date'])); ?></td>
                  <td><?php echo $Invreturn['ip_address']?></td> 
...
<script>
   ...
   $(function () {
       ...
       var url = '<?php echo base_url() ?>invoices/file_handling?id=<?php echo $invoice['returnid'] ?>';
       $('#fileupload').fileupload({
           ...
           formData: {'<?=$this->security->get_csrf_token_name()?>': crsf_hash},
               ...
               $.each(data.result.files, function (index, file) {
                   $('#files').append('<tr><td><a data-url="<?php echo base_url() ?>invoices/file_handling?op=delete&name=' + file.name + '&invoice=<?php echo $invoice['returnid'] ?>" class="aj_delete red"><i class="btn-sm fa fa-trash"></i></a> ' + file.name + ' </td></tr>');
               });
            ...
            <form class="payment">
               ...
               <div class="modal-footer">
                  <input type="hidden" class="form-control required"
                     name="tid" id="invoiceid" value="<?php echo $invoice['returnid'] ?>">
   ...
   <div class="modal-dialog">
      <div class="modal-content">
         ...
         <input type="hidden" class="form-control"
            name="tid" value="<?php echo $invoice['returnid'] ?>">
         <button type="button" class="btn btn-md btn-default"
            data-dismiss="modal"><?php echo $this->lang->line('Close'); ?></button>
      ...
      <div class="modal-content">
         ...
         <div class="modal-body" id="emailbody" style="display: none;">
            <form id="sendbill">
               ...
               <input type="hidden" class="form-control"
                  id="invoiceid" name="tid" value="<?php echo $invoice['returnid'] ?>">
               <input type="hidden" class="form-control"
