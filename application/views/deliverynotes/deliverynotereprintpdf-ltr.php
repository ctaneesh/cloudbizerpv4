        ...
        <thead>
            <tr>
                <td> 
                        ...
                        <?php 
                            $delnotenum = (!empty($delivery_note_number)) ? $delivery_note_number : $salesorder_id; 
                            $poreference = (!empty($salesorder_refrencedet['customer_reference_number'])) ? $salesorder_refrencedet['customer_reference_number'] : $customer_po_reference; 
                        ?>
                        <strong>Delivery Note No : <?= $delnotenum?></strong><br>
                        <strong>Your Reference No : <?= $poreference?></strong><br>
                        <?php if(!empty($salesorder_refrencedet['refdate'])){ ?>
                        <strong> Date: <?= date('d-m-Y', strtotime($salesorder_refrencedet['refdate']))?></strong><br>
                        <?php } ?>
                </td>
    ...
    <table cellpadding="0" cellspacing="0" class="plist maintable">
        ...
        <tr>           
            ...
            if($priceFlg==1){ ?>
                ...
                <?php if($discount>0){ ?>
                <td  with="50%" style="font-size: 12px;text-align:right;" class="d-none"><strong>Discount</strong></td>
                <?php } ?>
        ...
        foreach ($products as $row) {
            if($row['product_qty']>0)
            {
                if($deleverynote_status=='Assigned')
                {
                    ...
                    $prdqty=$row['product_qty'];
                    $prd_subtotal = $prd_subtotal + ($row['product_qty']*$row['product_price']);
                }
        ...
        <tr>
            ...
            <td style="text-align:right; font-size:16px;">
                ...
                if($priceFlg==1){ 
                    ...
                    $sub_total = ($discount>0) ? $subtotal-$totaldiscount : $subtotal;
                    ?>
                  <strong> Subtotal : <?=number_format($prd_subtotal, 2)?><br>
                    <!-- // erp2024 removed section 07-06-2024 -->
                     <?php if($discount>0){ ?>
                    Total Discount : <?=number_format($totaldiscount, 2)?><br> <?php } ?>
