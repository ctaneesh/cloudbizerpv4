...
<div class="invoice-box">
    ...
    <table class="plist" cellpadding="0" cellspacing="0">
        ...
        foreach ($list as $row) {
            ...
            // amountExchange($balance, 0, $this->aauth->get_user()->loc)
            $balance += $row['credit'] - $row['debit'];
            echo '<tr class="item' . $flag . '"><td>' . $row['date'] . '</td><td>' . $row['note'] . '</td><td>' . number_format($row['debit'], 2) . '</td><td>' . number_format($row['credit'], 2) . '</td><td>' . number_format($balance, 2) . '</td></tr>';
            $fill = !$fill;
        }
        ?>
    </table>
    ...
    <table class="subtotal">
        ...
        <tr>
            ...
            <td><?php echo $this->config->item('currency') . ' ' . number_format($balance, 2); ?></td>
        </tr>
