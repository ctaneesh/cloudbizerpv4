...
<div class="content-body">
    <div class="card">
        <div class="card-header border-bottom">
            ...
            <h4><?php echo $this->lang->line('Transaction')." #".$trans_ref_number; ?>&nbsp;<a href="<?= base_url('purchase/purchase_receipt_payment_edit?ref=' . $trans_ref_number.'&id='.$purchase['purchase_reciept_number']); ?>" class="btn btn-crud btn-sm btn-primary"><?php echo $this->lang->line('Edit'); ?></a></h4>
            ...
            <div class="card-body">
                <div id="invoice-customer-details" class="row">
                    ...
                    <div class="col-md-8 col-sm-12 responsive-text-right">
                        ...
                        if($purchase)
                        {
                            $receiptid = $purchase['purchase_reciept_number'];
                            $validtoken = hash_hmac('ripemd160', $receiptid, $this->config->item('encryption_key'));
                            $link = base_url('Invoices/costing?id=' . $receiptid . '&token=' . $validtoken);
                            $invdate = ($purchase['purchase_receipt_date']) ? dateformat($purchase['purchase_receipt_date']) : "" ;
                            echo '<p class="text-muted">'.$this->lang->line('Related Receipt').'</p>';
                            ...
                            echo '<p class="text-muted">'.$this->lang->line('Receipt Amount').' : '.number_format($purchase['bill_amount'],2).'</p>';
                            echo '<p class="text-muted">'.$this->lang->line('Paid Amount').' : '.number_format($purchase['purchase_paid_amount'],2).'</p>';
