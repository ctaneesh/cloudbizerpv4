...
<div class="invoice-box inset" >
    ...
    <thead>
        <tr>
            <td> <strong>Credit Note No : <?= $delevery_return_id?></strong></td>
            <td><strong>Credit Note Date : <?= dateformat($deleveryreturn_createddate)?></strong></td>
        </tr>
        </thead>
        <tbody>
        <tr>
            <td style="font-size: 14px; " width="50%">
            <strong>Credit Note For</strong><br><?=$client?><br>
    ...
    <table cellpadding="0" cellspacing="0" class="plist maintable">
        ...
        <tr>
            ...
            <td style="font-size: 12px;text-align:center;" with="50%"><strong>Delivered Quantity</strong></td>
            <td style="font-size: 12px;text-align:center;" with="50%"><strong>Return Quantity</strong></td>
            <td style="font-size: 12px;text-align:center;" with="50%"><strong>Damage Quantity</strong></td>
        ...
        $totaldiscount = 0;
        $totaltax = 0;
        $grand_product_price = 0;
        $order_discount = 0;
        foreach ($products as $row) {
            // echo '<tr class="itemx" style="font-size: 14px;"><td>' . $count . '</td><td>' . $row['product_code'] . '</td><td>'.$row['product_name'].'</td><td>'.$row['productunit'].'</td><td>' . $row['delivered_quantity'] . '</td><td>'.$row['return_quantity'].'</td><td>'.$row['damaged_quantity'].'</td><td>' . $row['product_price'] .'</td><td>'.$row['product_tax'].'</td><td>'.$row['totaltax'].'</td>';
            echo '<tr class="itemx" style="font-size: 14px;"><td>' . $count . '</td><td>' . $row['product_code'] . '</td><td>'.$row['product_name'].'</td><td>'.$row['productunit'].'</td><td style="text-align:center;">' . $row['delivered_quantity'] . '</td><td style="text-align:center;">'.$row['return_quantity'].'</td><td style="text-align:center;">'.$row['damaged_quantity'].'</td><td style="text-align:right;">' . $row['product_price'] .'</td>';
            ...
            $count=$count+1;
            $k++;
            $grand_product_price += intval($row['return_quantity'])*$row['product_price'];
            $subtotal = $row['subtotal']+$subtotal;
            $totaldiscount = $row['total_discount']+$totaldiscount;
            $totaltax = $row['totaltax']+$totaltax;
            $subgrand = $row['return_quantity']*$row['product_price'];
     ...
     <div style="text-align:right">
        ...
        ?>
        <h6>Subtotal : <?=number_format($grand_product_price, 2)?><br>
        <!-- // erp2024 removed section 07-06-2024 -->
        Total Product Discount : <?=number_format($totaldiscount, 2)?><br>
        Order Discount : <?=number_format($order_discount, 2)?><br>
        <!-- // erp2024 removed section 07-06-2024 -->
        <!-- Grand Total : <?php echo number_format(($subtotal+$totaltax), 2); ?> -->
        Grand Total : <?php echo number_format(($subtotal+$totaltax)-$order_discount, 2); ?>
        </h6>
     </div>
     <!-- // erp2024 removed section 07-06-2024 -->
        ...
        <tbody>
            <tr>
                ...
                <!-- <td style="text-align: right;font-size: 14px; padding-left:50px;" width="50%">
                    <h6>Subtotal : <?=number_format($subtotal,2)?><br>
