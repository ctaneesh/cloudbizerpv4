...
<head>
    ...
    <title>Print Delivery Return</title>
...
<div class="invoice-box inset" >
   ...
   <?php 
    // echo "<pre>";
    // print_r($order_discount);
    // die();
   ?>
    <table>
    <thead>
        <tr>
            <td> <strong>Delivery Return No. : <?= $delevery_note_id?></strong></td>
            <td><strong>Delivery Return Date : <?= date('d-m-Y',strtotime($deleveryreturn_createddate))?></strong></td>
        </tr>
        </thead>
        <tbody>
        <tr>
    ...
    <table cellpadding="0" cellspacing="0" class="plist maintable">
       ...
       $count=1;
        ...
        foreach ($products as $row) {
            ...
            echo '<tr class="itemx" style="font-size: 14px;"><td>' . $count . '</td><td>' . $row['product_code'] . '</td><td>'.$row['product_name'].'</td><td>'.$row['productunit'].'</td><td style="text-align:center;">' . $row['delivered_quantity'] . '</td><td style="text-align:center;">'.$row['return_quantity'].'</td><td style="text-align:center;">'.$row['damaged_quantity'].'</td><td style="text-align:right;">' . number_format($row['product_price'],2) .'</td>';
            ...
            $product_total += ($row['product_price']*$row['return_quantity']);
        }
        ?>
    </table>
     ...
     <div style="text-align:right">
        <?php 
            ...
            $grand_discount =  $order_discount+$totaldiscount;
            $grandtotal = (($subtotal)-$order_discount);
        ?>
        <h6>Subtotal : <?=number_format($product_total, 2)?><br>
        <!-- // erp2024 removed section 07-06-2024 -->
        Product Discount : <?=number_format($totaldiscount, 2)?><br>
        Order Discount : <?=number_format($order_discount, 2)?><br>
        Total Discount : <?=number_format($grand_discount, 2)?><br>
        <!-- Total Tax : <?=$totaltax?><br> -->
        <!-- // erp2024 removed section 07-06-2024 -->
        Grand Total : <?php echo number_format(($grandtotal+$totaltax), 2); ?>
