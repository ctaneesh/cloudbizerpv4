...
<head>
    ...
    <title>Print  #<?php echo $invoice['tid'] ?></title>
    <style>
        body {
            color: #2B2000;
   ...
   <hr>
    ...
    <thead>
        <tr>
            <?php
                ...
                $reciept = 'Purchase Return';
                $prefix = get_prefix();
                $purchasereturn_prefix =  $prefix['purchasereturn_prefix'];
            ?>
            <td style="text-align:left; vertical-align: middle;"><h3><?=$reciept?></h3></td>
        
            <td style="text-align:right;"><strong>Purchase Return No : <?= $invoice['receipt_return_number']?></strong><br> <strong>Date : <?= date('d-m-Y', strtotime($invoice['return_date']))?></strong></td>
        </tr>
        </thead>
    ...
    <table class="plist maintable" cellpadding="0" cellspacing="0">
        ...
        foreach ($products as $row) {
            ...
            $prdprice = ($row['price'] * $row['quantity']);
            $sub_t += $prdprice;
            ...
            echo '<td style="text-align:right;">' . number_format($prdprice,2) . '</td></tr>';
    ...
    <table class="subtotal">
        ...
        <tr>
           ...
           <?php  } ?>
                ...
                <p><?php echo $this->lang->line('Payment Status') . ' : <strong>'. $this->lang->line(ucwords($invoice['payment_status'])); ?></strong></p>
                    ...
                    if (!$general['t_type']) {
                        // echo '<br><p>' . $this->lang->line('Total Amount') . ' : <strong>' . number_format($sub_t,2) . '</p><br><p>';
                        ...
                        // echo $this->lang->line('Paid Amount') . ': ' . amountExchange($invoice['payment_recieved_amount'], $invoice['multi'], $invoice['loc']);
                       echo "<br>". $this->lang->line('Paid Amount') . ' : <strong>' . number_format($invoice['payment_recieved_amount'],2).'</strong><br>';
                    }
        ...
        <tr class="f_summary">
            ...
            <td style="text-align:right"><?php echo number_format($sub_t,2); ?></td>
        ...
        if ($invoice['shipping'] > 0) {
            ...
            <td style="text-align:right">' . $invoice['shipping'] . '</td>
        </tr>';
        }
        ?>
        <tr>
            ...
            // $totaldiscount += $row['totaldiscount'];
            // $prdprice = ($row['price'] * $row['qty']);
            // $sub_t += $prdprice;
            $grandtotal = $sub_t - $totaldiscount;
            $rming = $grandtotal - $invoice['payment_recieved_amount'];
