...
<div id='printbox'>
    ...
    <table class="inv_info">
        <?php   if ($loc['tax_id']) {      ?> <tr>
            <td><?php echo $this->lang->line('Tax') . ' ID: '  ?></td>
            <td><?php echo $loc['tax_id'] ?></td>
        </tr>
        <?php } ?>
        <tr>
    ...
    <table class="inv_info">
        ...
        <?php } else if ($invoice['taxstatus'] == 'igst') {
            ...
            <tr>
                ...
                <td><b><?php echo number_format($row['tax'],2);
                //echo amountExchange($invoice['tax'], $invoice['multi']) ?></b>
                    (<?= amountFormat_general($row['tax']) ?>%)
                </td>
            </tr>
        <?php } ?>
        <tr>
                <td><b><?php echo $this->lang->line('Currency') ?></b></td>
                <td><b><?php  echo $this->config->item('currency');
                //amountExchange($invoice['total'], $invoice['multi'], $invoice['loc']) ?></b></td>
            </tr>
        <tr>
            <td><b><?php echo $this->lang->line('Total Tax') ?></b></td>
            <td><b><?php echo number_format($invoice['tax'],2);
            //amountExchange($invoice['tax'], $invoice['multi'], $invoice['loc']) ?></b></td>
        </tr>
        <tr>
            <td><b><?php echo $this->lang->line('Total Discount') ?></b></td>
            <td><b><?php 
            echo number_format($invoice['discount'],2);
            ///echo amountExchange($invoice['discount'], $invoice['multi'], $invoice['loc']) ?></b></td>
        </tr>
        <tr>
            <td><b><?php echo $this->lang->line('Total') ?></b></td>
            <td><b><?php  echo number_format($invoice['total'],2);
            //amountExchange($invoice['total'], $invoice['multi'], $invoice['loc']) ?></b></td>
        </tr>
