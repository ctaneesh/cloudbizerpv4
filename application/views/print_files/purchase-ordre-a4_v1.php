...
<head>
    ...
    <title>Purchase Order  <?php echo $prefix['po_prefix'].$invoice['tid'] ?></title>
    <style>
        body {
            color: #2B2000;
   ...
   <hr>
    ...
    <thead>
        <tr>
            ...
            <td style="text-align:right;"><strong>Purchase Order : <?= $invoice['purchase_number']?></strong><br> <strong>Purchase Order Date : <?= dateformat($invoice['purchase_order_date'])?></strong><br>
            <strong>Purchase Order Due Date : <?= dateformat($invoice['duedate'])?></strong></td>
        </tr>
        </thead>
    ...
    <table class="subtotal">
        ...
        <tr>
           ...
           <?php  } ?>
                ...
                <p><?php
                    if (!$general['t_type']) {
                        echo '<br><p>' . $this->lang->line('Total Amount') . ' : <strong>' . number_format($invoice['order_total'],2) . '</p><br><p>';
                        // echo '<br><p>' . $this->lang->line('Total Amount') . ': ' . amountExchange($invoice['total'], $invoice['multi'], $invoice['loc']) . '</p><br><p>';
                        // echo '<br><p>' . $this->lang->line('Total Amount') . ': ' . amountExchange($invoice['total'], $invoice['multi'], $invoice['loc']) . '</p><br><p>';
                        if (@$round_off['other']) {
                            $final_amount = round($invoice['order_total'], $round_off['active'], constant($round_off['other']));
        ...
        if ($invoice['shipping'] > 0) {
            ...
            <td style="text-align:right">' . number_format($invoice['shipping_charge'],2) . '</td>
        </tr>';
        }
        ?>
        <tr>
            <td><?php echo $this->lang->line('Balance Due') ?></td>
            <td style="text-align:right"><strong><?php $rming = $invoice['order_total'] - $invoice['paid_amount'];
    if ($rming < 0) {
        $rming = 0;
    }
    if (@$round_off['other']) {
