   ...
   <hr>
    ...
    <thead>
        <tr>
            ...
            ?>
            <td style="text-align:left; vertical-align: middle;"><h3>Invoice Preview</h3></td>
        
            <td style="text-align:right;"><strong>Invoice No : #<?= $invoice['tid']?></strong><br> <strong>Invoice Date : <?= date('d-m-Y', strtotime($invoice['invoicedate']))?></strong><br>
            <strong>Invoice Due Date : <?= date('d-m-Y', strtotime($invoice['invoiceduedate']))?></strong></td>
        </tr>
        </thead>
    ...
    <table class="plist maintable" cellpadding="0" cellspacing="0">
        ...
        foreach ($products as $row) {
            ...
            if ($invoice['tax'] > 0) {
                $cols++;
                echo '<td style="width:16%;">' . number_format($row['totaltax'],2) . ' <span class="tax">(' . amountFormat_s($row['tax']) . '%)</span></td>';
            }
    ...
    <table class="subtotal">
        ...
        <tr>
            <td class="myco2" style="width:50%;" rowspan="<?php echo number_format($sub_t_col,2) ?>"><br>
           ...
           <?php  } ?>
                ...
                <p><?php echo $this->lang->line('Status') . ' : <strong>'. $this->lang->line(ucwords($invoice['status'])) . '</strong></p>';
                    ...
                    if (!$general['t_type']) {
                        echo '<br><p>' . $this->lang->line('Total Amount') . ' : <strong>' . ($invoice['total']) . '</p><br><p>';
