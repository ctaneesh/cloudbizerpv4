...
class Products_model extends CI_Model
{
    ...
    var $column_order = array(null, 'cberp_products.product_name', 'cberp_products.onhand_quantity', 'cberp_products.product_code', 'cberp_product_category.title', 'cberp_products.product_price', null); //set column field database for datatable orderable
    var $column_search = array('cberp_products.product_name', 'cberp_products.product_code', 'cberp_product_category.title', 'cberp_store.title'); //set column field database for datatable searchable
    var $order = array('cberp_products.pid' => 'desc'); // default order
    ...
    {
        $this->db->select('cberp_products.*,cberp_product_category.title AS c_title,cberp_store.title');
        ...
        if ($sub) {
            ...
            if ($this->input->post('group') != 'yes') $this->db->where('cberp_products.merge', 0);
            ...
            if ($w) {
                ...
                if ($id > 0) {
                    $this->db->where("cberp_store.id = $id");
    ...
    public function edit($pid, $catid, $warehouse, $product_name, $product_code, $product_price, $factoryprice, $taxrate, $disrate, $product_qty, $product_qty_alert, $product_desc, $image, $unit, $barcode, $code_type, $sub_cat = '', $b_id = '', $vari = null, $serial = null, $wdate = null)
    {
        ...
        if (isset($serial['old'])) {
            $this->db->delete('cberp_product_serials', array('product_id' => $pid,'status'=>0));
            $serial_group = array();
