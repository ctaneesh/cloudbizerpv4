...
class Settings_model extends CI_Model
{
    public function company_details($id)
    {
        ...
        $this->db->select('*');
        $this->db->from('cberp_system');
        $this->db->where('id', $id);
        $query = $this->db->get();
        return $query->row_array();
    }

    public function update_company($id, $name, $phone, $email, $address, $city, $region, $country, $postbox, $tax_id,$data_share,$foundation)
    {
        $data = array(
            'cname' => $name,
            'phone' => $phone,
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('cberp_system')) {
            configured_data();
    ...
    public function update_billing($id, $invoiceprefix, $tax_id, $taxstatus, $lang)
    {
        $data = array(
            'tax_id' => $tax_id,
            'tax' => $taxstatus,
    ...
    public function update_prefix($invoiceprefix, $q_prefix, $p_prefix, $r_prefix, $s_prefix, $t_prefix, $o_prefix, $pos_prefix,$dnote_prefix,$dnote_return_prefix,$invoicereturnprefix,$leadprefix,$salesorderprefix,$t_suffix,$recieptprefix)
    {
        $data = array(
            ...
            'method' => $t_prefix,
            'other' => $o_prefix,
            'suffix' => $recieptprefix
        );
        
        
        $dnotdata = array(
            'name' => $dnote_prefix,
            'key1' => $dnote_return_prefix,
            'url' => $invoicereturnprefix,
            'method' => $leadprefix,
    ...
    public function add_term($title, $type, $term)
    {
        ...
        if ($this->db->insert('cberp_terms', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('ADDED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function edit_term($id, $title, $type, $term)
    {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('cberp_terms', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    {
        $this->db->select('cberp_system.currency,univarsal_api.*');
        $this->db->from('cberp_system');
        $this->db->where('univarsal_api.id', 4);
        $this->db->where('cberp_system.id', 1);
        $this->db->join('univarsal_api', 'cberp_system.id = 1', 'left');
        $query = $this->db->get();
        return $query->row_array();
    ...
    public function delete_terms($id)
    {
        ...
        $query = $this->db->get();
        if ($query->num_rows() > 1) {
            return $this->db->delete('cberp_terms', array('id' => $id));
        } else {
            return false;
        }
    ...
    {
        return $this->db->delete('cberp_config', array('id' => $id, 'type' => 2));
    }

    public function update_tax($id, $tax_id, $taxstatus, $tdirection)
    {
        $data = array(
            'tax_id' => $tax_id,
            'tax' => $taxstatus
    ...
    public function custom_fields($id = 0)
    {
        ...
        if ($id) {
            $this->db->select('*');
            $this->db->from('cberp_custom_fields');
            $this->db->where('id', $id);
            $query = $this->db->get();
            return $query->row_array();
        } else {
    ...
    public function custom_field_add($f_name, $f_type, $f_module, $f_view, $f_required, $f_placeholder, $f_description)
    {
        ...
        if ($this->db->insert('cberp_custom_fields', $data)) {
            echo json_encode(array('status' => 'Success', 'message' =>
    ...
    public function custom_field_edit($id, $f_name, $f_view, $f_required, $f_placeholder, $f_description)
    {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        if ($this->db->update('cberp_custom_fields')) {
            echo json_encode(array('status' => 'Success', 'message' =>
                $this->lang->line('UPDATED')));
        } else {
            echo json_encode(array('status' => 'Error', 'message' =>
    ...
    public function get_default_validity()
    {
        $this->db->select('*');
        $this->db->from('cberp_default_validity');
        $query = $this->db->get();
        return $query->row_array();
    }
