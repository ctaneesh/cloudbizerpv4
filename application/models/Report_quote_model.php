...
class Report_quote_model extends CI_Model
{ 
    var $table = 'cberp_quotes';  
    var $column_order = array(null, 'cberp_quotes.id','cberp_quotes.tid','cberp_quotes.invoicedate','cberp_quotes.invoiceduedate','cberp_quotes.invoiceduedate','cberp_quotes.total','cberp_quotes.approved_dt','cberp_employees.name','cberp_customer_leads.lead_id','create_employee.name','cberp_customer_leads.created_date','lead_employee.name');
    var $column_search = array('cberp_quotes.id','cberp_quotes.tid','cberp_quotes.invoicedate','cberp_quotes.invoiceduedate','cberp_quotes.invoiceduedate','cberp_quotes.total','cberp_quotes.approved_dt','cberp_employees.name','cberp_customer_leads.lead_id','create_employee.name','cberp_customer_leads.created_date','lead_employee.name');
    var $order = array('cberp_quotes.tid' => 'desc');

    // $this->db->select('cberp_quotes.id,cberp_quotes.tid,cberp_quotes.invoicedate,cberp_quotes.invoiceduedate,cberp_quotes.invoiceduedate,cberp_quotes.total,cberp_quotes.status,cberp_quotes.convertflg,cberp_quotes.approvalflg,cberp_quotes.refer,cberp_quotes.customer_reference_number,cberp_customers.name,cberp_customers.customer_id as customerid,cberp_quotes.prepared_flg,cberp_quotes.approved_dt,cberp_employees.name as employeename');
    ...
    public function quote_details($id)
    {
        ...
        $this->db->select('cberp_quotes.*,cberp_quotes.status as quotestatus,cberp_quotes.id AS iid,SUM(cberp_quotes.shipping + cberp_quotes.ship_tax) AS shipping,cberp_customers.*,cberp_quotes.loc as loc,cberp_customers.customer_id AS cid,cberp_terms.id AS termid,cberp_terms.title AS termtit,cberp_terms.terms AS terms');
        $this->db->from($this->table);
        $this->db->where('cberp_quotes.id', $id);
        //  if ($this->aauth->get_user()->loc) {
        //     $this->db->where('cberp_quotes.loc', $this->aauth->get_user()->loc);
        // } elseif (!BDATA) {
        //     $this->db->where('cberp_quotes.loc', 0);
        // }
        $this->db->join('cberp_customers', 'cberp_quotes.csd = cberp_customers.customer_id', 'left');
        $this->db->join('cberp_terms', 'cberp_terms.id = cberp_quotes.term', 'left');
        $query = $this->db->get();
    ...
    {
        $this->db->select('cberp_quotes_items.*, cberp_products.onhand_quantity AS totalQty, cberp_products.alert_quantity, cberp_products.product_code, cberp_products.product_cost,cberp_products.product_name');
        $this->db->from('cberp_quotes_items');
        $this->db->join('cberp_products', 'cberp_products.pid = cberp_quotes_items.pid', 'left');
    ...
    private function _get_datatables_query($eid)
    {
       ...
       $filter_expiry_date_to = !empty($this->input->post('filter_expiry_date_to')) ? date('Y-m-d',strtotime($this->input->post('filter_expiry_date_to'))) : "";
        ...
        $this->db->select('
        cberp_quotes_items.*,
        cberp_products.product_cost,
         cberp_products.product_name,  
        cberp_quotes.id,
        cberp_quotes.tid,
        cberp_quotes.invoicedate,
        cberp_quotes.invoiceduedate,
        cberp_quotes.total,
        cberp_quotes.status,
        cberp_quotes.convertflg,
        cberp_quotes.approvalflg,
        cberp_quotes.refer,
        cberp_quotes.customer_reference_number,       
        cberp_customers.name as customer_name,
        cberp_customers.customer_id as customerid,
        cberp_quotes.prepared_flg,
        cberp_quotes.approved_dt,
        cberp_employees.name as employeename,
        cberp_customer_leads.lead_id,
        create_employee.name as created_name,
        cberp_customer_leads.created_date,
        lead_employee.name as leadname
    ');
    
    $this->db->from('cberp_quotes_items');
    
    // Join with cberp_quotes using LEFT JOIN to keep all rows from cberp_quotes_items
    $this->db->join('cberp_quotes', 'cberp_quotes.id = cberp_quotes_items.tid', 'inner');
    
    // Join with cberp_customers on cberp_quotes.csd
    $this->db->join('cberp_customers', 'cberp_quotes.csd = cberp_customers.customer_id', 'left');

    $this->db->join('cberp_products', 'cberp_quotes_items.pid = cberp_products.pid', 'left');
    
    // Join with cberp_employees for approved_by in cberp_quotes
    $this->db->join('cberp_employees', 'cberp_employees.id = cberp_quotes.approved_by', 'left');
    
    // Join with cberp_employees again as create_employee for created_by in cberp_quotes
    $this->db->join('cberp_employees as create_employee', 'create_employee.id = cberp_quotes.created_by', 'left');
    
    // Join with customer_leads on lead_id in cberp_quotes
    $this->db->join('cberp_customer_leads', 'cberp_customer_leads.lead_id = cberp_quotes.lead_id', 'left');
    
    // Join with cberp_employees again as lead_employee on created_by in customer_leads
    $this->db->join('cberp_employees as lead_employee', 'lead_employee.id = cberp_customer_leads.created_by', 'left');
        ...
        if (!empty($filter_expiry_date_from) && !empty($filter_expiry_date_to)) {
            $this->db->where("cberp_quotes.invoiceduedate BETWEEN '$filter_expiry_date_from' AND '$filter_expiry_date_to'");
        }
        ...
        if (isset($_POST['order'])) // here order processing
        {
            ...
            $order = array('cberp_quotes.id' => 'desc');
        }
    ...
    //sales order 
    public function insert_to_sales_order_items($id,$last_insert_id) {
        $this->db->select('*');
        $this->db->from('cberp_quotes_items');
        $this->db->where('tid', $id);
        $query = $this->db->get();
    ...
    {
        $this->db->select('cberp_sales_orders_items.*, cberp_products.onhand_quantity AS totalQty, cberp_products.product_name, cberp_products.alert_quantity, cberp_products.product_code, cberp_products.unit, cberp_products.product_price, cberp_product_ai.min_price AS product_lowest_price, cberp_product_ai.max_disrate AS product_max_discount, cberp_sales_orders_items.delivered_qty AS deliveredqty, cberp_sales_orders_items.remaining_qty AS remainingqty, cberp_sales_orders_items.transfered_qty AS trasferedqty, cberp_sales_orders_items.ordered_qty AS orderedqty, cberp_customer_lead_items.qty AS leadqty, cberp_customer_lead_items.id AS lead_id, cberp_customer_leads.created_date AS leaddate, cberp_quotes.id AS quote_id, cberp_quotes.invoicedate AS quotedate, cberp_quotes_items.qty AS quoteqty, (cberp_product_to_store.stock_qty + cberp_product_to_store.intransit_qty) AS onhandqty');
        $this->db->from('cberp_sales_orders_items');
    ...
    public function update_quote_status($quote_id, $salestid, $salesorder_id) {
        ...
        if ($query->num_rows() > 0) {
            ...
            $sql_update2 = "
                ...
                WHERE id = ?
            ";
            $this->db->query($sql_update2, array($salestid, $salesorder_id));
        } else {
    ...
    public function get_customer_by_quoteid($quote_id)
    {
        ...
        $this->db->from('cberp_quotes');
        $this->db->join('cberp_customers', 'cberp_customers.customer_id = cberp_quotes.csd');
    ...
    public function quote_details_byid($id)
    {
    ...
    public function product_history($productid,$customerid){
        ...
        $this->db->order_by('cberp_quotes.invoicedate', 'DESC');        
        // Execute the query
        $query = $this->db->get();
        // die($this->db->last_query());
        return $query->result_array();
    ...
    public function get_quote_number_from_salesorder($id)
    {
        $this->db->select('cberp_sales_orders.quote_id');
        $this->db->from('cberp_sales_orders');
        $this->db->where('id', $id);
        $query = $this->db->get();
        // die($this->db->last_query());
    ...
    public function get_quote_details($quote_id) {
        $this->db->select('cberp_customer_lead_items.qty AS leadqty,
                           ...
                           cberp_quotes.invoicedate as quotedate, 
