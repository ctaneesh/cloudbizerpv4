...
class Projects_model extends CI_Model
{
    ...
    var $column_order = array('cberp_projects.status', 'cberp_projects.name', 'cberp_projects.edate', 'cberp_projects.worth', null, null);
...
//project
    ...
    private function _project_datatables_query($cday = '', $eid = '')
    {
        ...
        if ($eid) {
            ...
            $this->db->join('cberp_project_meta', 'cberp_projects.id = cberp_project_meta.pid', 'left');
            $this->db->where('cberp_project_meta.meta_key', 19);
            $this->db->where('cberp_project_meta.meta_data', $eid);
        }
        if ($cday) {
    ...
    public function addproject($name, $status, $priority, $progress, $customer, $sdate, $edate, $tag, $phase, $content, $budget, $customerview, $customer_comment, $link_to_cal, $color, $ptype, $employee)
    {
        ...
        $this->db->insert('cberp_projects', $data);
    ...
    public function setnote($id, $stat)
    {
        ...
        $this->db->set($data);
        $this->db->where('id', $id);
        return $this->db->update('cberp_projects');
    }
    ...
    {
        $this->db->delete('cberp_todolist', array('related' => 1, 'rid' => $id));

        return $this->db->delete('cberp_projects', array('id' => $id));
    }
    ...
    {
        $this->db->delete('cberp_milestones', array('id' => $mid));
    }
    ...
    public function task_stats($id)
    {
        $query = $this->db->query("SELECT
				...
				FROM cberp_todolist WHERE related=1 AND rid=$id");
    ...
    {
        $this->db->select('cberp_employees.*,cberp_project_meta.key4');
        $this->db->from('cberp_project_meta');
        $this->db->where('cberp_project_meta.pid', $id);
        $this->db->where('cberp_project_meta.meta_key', 29);
        $this->db->join('cberp_employees', 'cberp_employees.id = cberp_project_meta.meta_data', 'left');
        $this->db->join('cberp_users', 'cberp_employees.id = cberp_users.id', 'left');
        $this->db->order_by('cberp_users.roleid', 'DESC');
        $query = $this->db->get();
    ...
    public function clockin($id, $eid)
    {
        ...
        if (!$emp['key3'] AND $emp['pid']) {
            ...
            $this->db->update('cberp_project_meta');
