    ...
    $('#productname-' + cvalue).autocomplete({
        ...
        select: function (event, ui) {
            ...
            id = id_arr.split("-");
             ...
             var newProductID = ui.item.data[2];
...
$("#addproduct1").on('click', function () {
    ...
    var lastSerialNumber = parseInt($('.serial-number:last').text()) || 0; 
    var nextSerialNumber = lastSerialNumber+1; 
    var data = '<tr><td class="text-center serial-number">'+nextSerialNumber+'</td><td><input type="text" class="form-control wid95per" name="product_code[]" placeholder="Enter Product Code" id="code-'+ cvalue +'"></td>><td><input type="text" class="form-control wid95per" name="product_name[]" placeholder="Enter Product name or Code" id="productname-'+ cvalue +'"></td><td><input type="text" class="form-control req amnt wid65per" name="product_qty[]" id="amount-'+ cvalue +'" onkeypress="return isNumber(event)" autocomplete="off" value="0" ><input type="hidden" class="pdIn" name="pid[]" id="pid-'+ cvalue +'" > </td><td class="text-center1"><button type="button" data-rowid="'+ cvalue +'" class="btn btn-sm btn-secondary removeProd1" title="Remove" > <i class="icon-trash"></i> </button> </td> </tr>';
    // var data = '<tr><td class="text-center serial-number">'+nextSerialNumber+'</td><td><input type="text" class="form-control wid95per" name="product_code[]" placeholder="Enter Product Code" id="code-'+ cvalue +'"></td>><td><input type="text" class="form-control wid95per" name="product_name[]" placeholder="Enter Product name or Code" id="productname-'+ cvalue +'"></td><td><input type="text" class="form-control req amnt wid65per" name="product_qty[]" id="amount-'+ cvalue +'" onkeypress="return isNumber(event)" autocomplete="off" value="0" ><input type="hidden" class="pdIn" name="pid[]" id="pid-'+ cvalue +'" > </td><td class="text-center1"><button type="button" data-rowid="'+ cvalue +'" class="btn btn-sm btn-secondary removeProd1" title="Remove" onclick="removeTr('+ cvalue +')"> <i class="icon-trash"></i> </button> </td> </tr>';
...
//remove productrow
    ...
    $( '#saman-row' ).on( 'click', '.removeProd1', function () {
        swal.fire({
            title: "Are you sure?",
    ...
    $('#productname-0').autocomplete({
        ...
        select: function (event, ui) {
            var names = ui.item.data;
             //check duplicate entry starts
            var newProductCode = ui.item.data[2]; // New selected product name
            var duplicateFound = false;      
            var totalRows = parseInt($('#ganak').val()); // total number of rows
            for (var i = 0; i <= totalRows; i++) {
                if ($('#code-' + i).length) {
            ...
            if (duplicateFound) {                
                duplicate_message(cvalue,newProductCode);
                return false;
            }
             //check duplicate entry ends
            $('#pid-0').val(names[1]);
    ...
    $('#code-0').autocomplete({
        source: function (request, response) {
            $.ajax({
                ...
                success: function (data) {
                    response($.map(data, function (item) {
                        return {
                            label: item[2]+" ("+item[0]+")", // product_name
                            value: item[2], // product_name
                            data: item
                        };
                    }));
                }
        ...
        select: function (event, ui) {
            ...
            if (duplicateFound) {                
             ...
             //check duplicate entry ends
            $('#pid-0').val(names[1]);
            $('#productname-0').val(names[0]);
            $('#amount-0').val(1);
        }
    });
    
    function autocompletePrdts(ival){
        $('#productname-'+ival).autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: baseurl + 'enquiry/search',
                    dataType: "json",
            ...
            select: function (event, ui) {
                ...
                if (duplicateFound) {                
                    duplicate_message(ival,newProductID);
                    return false;
                }
                //check duplicate entry ends
                $('#pid-'+ival).val(names[1]);
                $('#code-'+ival).val(names[2]);
            }
        });
        $('#code-'+ival).autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: baseurl + 'enquiry/search',
                    dataType: "json",
